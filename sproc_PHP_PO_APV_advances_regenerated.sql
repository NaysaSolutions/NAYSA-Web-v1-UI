    ...
    begin
        ...
        select @_result = (
            select
                ...
                s.appliedAdvAmount,
                s.poJoNo as advpoNo,
                s.poAmount as advpoAmount,
                s.vatAmount as advpoVatAmount,
                s.vatAmount as advpoAtcAmount
            from dbo.vw_PO_Mode_POAPV_Summary s
                join payterm_ref pt
                    on pt.payterm_code = s.paytermCode
            where (@_branchCode = '' or s.branchCode = @_branchCode)
              and (@_vendCode = '' or s.vendCode = @_vendCode)
              and upper(ltrim(rtrim(isnull(pt.advances, '')))) in ('Y', 'YES')
            order by s.branchCode, s.poJoNo desc
            for json path, include_null_values
        );
