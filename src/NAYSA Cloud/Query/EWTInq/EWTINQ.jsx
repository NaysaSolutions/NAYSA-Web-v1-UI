...
export default function EWTINQ() {
  ...
  const handleViewTop = useCallback(
    (row) => {
      const selectedVendCode = String(row?.vendCode || "").trim().toUpperCase();
      const selectedAtcCode = String(row?.atcCode || "").trim().toUpperCase();
      const filteredRows = originalRows.filter(
        (r) =>
          String(r?.vendCode || "").trim().toUpperCase() === selectedVendCode &&
          String(r?.atcCode || "").trim().toUpperCase() === selectedAtcCode
      );
