...
export default function INTAXINQ() {
  ...
  const doFind = useCallback(async () => {
    ...
    try {
      ...
      if (safeRows.length === 0 && safeRowsAtt.length === 0) {
        ...
        useSwalErrorAlert("Input VAT Inquiry", "No records found.");
        return;
      }
  ...
  const doExport = useCallback(async () => {
    if (!Array.isArray(rows) || rows.length === 0) {
      useSwalErrorAlert("Input VAT Inquiry", "No data available.");
      return;
    }
    ...
    async (kind) => {
      ...
      try {
        ...
        if (!tblAtt || tblAtt.length === 0) {
          useSwalErrorAlert("Input VAT Inquiry", "No attachment data found.");
          return;
        }
  ...
  const doGenerate = useCallback(
    (kind) => {
      ...
      try {
        ...
        if (!datText) {
          useSwalErrorAlert("Input VAT Inquiry", `No ${kind} file data found.`);
          return;
        }
        ...
        useSwalErrorAlert("Input VAT Inquiry", `Failed to generate ${kind} file.`);
      } finally {
        updateState({ isLoading: false, showGenerateMenu: false });
      }
    },
    [rows, tblFSLP_dat, tblFSLP_fileName, normalizeDat]
  );
