...
function RMStockCardQuery() {
  ...
  const setupQuery = useQuery({
    queryKey: ["rm-stock-card-setup", defaultInventorySetup],
    queryFn: async () => {
      const response = await apiClient.get("/rm/inventory/stock-card/setup");
  ...
  const balanceQuery = useQuery({
    queryKey: ["rm-stock-card-balance", inventorySetup, shouldLoadBalance, balanceFilters],
  ...
  const stockCardQuery = useQuery({
    queryKey: ["rm-stock-card-movement", shouldLoadStockCard, stockCardFilters],
    enabled: shouldLoadStockCard > 0,
    queryFn: async () => {
      const response = await apiClient.get("/rm/inventory/stock-card/stock-card", {
  ...
  const stockStatusQuery = useQuery({
    queryKey: ["rm-stock-status", shouldLoadStockStatus],
    enabled: shouldLoadStockStatus > 0,
    queryFn: async () => {
      const response = await apiClient.get("/rm/inventory/stock-card/stock-status", {
...
export default RMStockCardQuery;
