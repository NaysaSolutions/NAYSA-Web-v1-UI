...
const INITIAL_FORM = {
  jobCode: "", jobName: "", uomCode: "", 
  acctCode: "", acctName: "", active: "Y",
};
...
const JobCodeRef = () => {
  ...
  const openMobileActionSheet = (row) => { setSelectedMobileRow(row); setIsMobileActionSheetOpen(true); };
  const closeMobileActionSheet = () => { setIsMobileActionSheetOpen(false); setSelectedMobileRow(null); };
  ...
  const { mutate: saveJobCode, isLoading: isSaving } = useMutation({
    mutationFn: async (payload) => await apiClient.post("/upsertJobCode", payload),
    onSuccess: (response) => {
  ...
  const findDuplicateJobName = (jobName = formData.jobName) => {
    ...
    return jobCodes.find((item) => {
      ...
      const sameCode = normalizeText(item?.jobCode) === normalizeText(selectedJobCode || formData.jobCode);
  ...
  const handleCheckDuplicateCode = async (code) => {
    ...
    try {
      ...
      const parsedData = JSON.parse(response?.data?.data?.[0]?.result || '{"result":"0"}');

      if (parsedData.result === "1") {
        resetForm();
        return useSwalErrorAlertAPI(`Duplicate Job Code: ${code}`, `This code is already in use.`);
      }
  ...
  const resetForm = () => {
    ...
    setIsEditing(false);
    if (formTopRef.current) formTopRef.current.scrollIntoView({ behavior: 'smooth', block: 'end' });
  };
  ...
  const handleEdit = (row) => {
    ...
    setRegistrationInfo({
      registeredBy: row.registeredBy, registeredDate: row.registeredDate,
      lastUpdatedBy: row.lastUpdatedBy, lastUpdatedDate: row.lastUpdatedDate
    });
    setIsEditing(true);
  ...
  const handleDelete = async (row) => {
    try {
      ...
      if (String(parsedData.result) === "1") {
        return useSwalErrorAlertAPI(`Cannot Delete`, `Job Code ${row.jobCode} is used in transactions.`);
      }

      const confirm = await useSwalDeleteConfirm("Confirm Delete", `Are you sure you want to delete Code: ${row.jobCode}?`);
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (row) => (
        <div className="flex gap-2 justify-center w-full">
          <button 
            onClick={() => isMobile ? openMobileActionSheet(row) : handleEdit(row)} 
            className="global-ref-td-button-edit-ui"
          >
            <FontAwesomeIcon icon={faEdit} />
          </button>
          <button 
            onClick={() => isMobile ? openMobileActionSheet(row) : handleDelete(row)} 
    ...
    { key: "jobCode", label: "Job Code", sortable: true, width: 120, pinned: true },
    ...
    { key: "active", label: "Active", sortable: true, width: 80, render: (row) => (row.active === "Y" ? "Yes" : "No") },
  ], [handleDelete, isMobile]); // isMobile added to dependencies
    ...
    <div className="global-ref-main-div-ui">
      ...
      <div className="global-ref-header-ui">
        <div className="w-full flex flex-col md:grid md:grid-cols-2 items-center">
          <h1 className="global-ref-headertext-ui">{reftables[docType] || "Job Code Reference"}</h1>
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-6">
            ...
            <FieldRenderer
              ...
              disabled={!isEditing}
              onChange={(v) => { allowedDuplicateJobNameRef.current = ""; updateForm({ jobName: v }); }}
          ...
          <div className="space-y-6">
            ...
            <FieldRenderer
              label="Account Code" required type="lookup"
              value={formData.acctCode && formData.acctName ? `${formData.acctCode} - ${formData.acctName}` : formData.acctCode}
              disabled={!isEditing}
      ...
      <div className="global-tran-table-main-div-ui mt-4">
        <SearchGlobalReferenceTable
          docType={docType} columns={columns} data={jobCodes}
          onRowDoubleClick={handleEdit} itemsPerPage={200}
          onMobileRowOpen={openMobileActionSheet} isLoading={isListLoading}
          onRefresh={() => queryClient.invalidateQueries({ queryKey: ["jobCodeList"] })}
          autoFillGrid={true}
        />
      </div>
      ...
      {isMobileActionSheetOpen && (
        <div className="fixed inset-0 z-[120] md:hidden">
          <div className="absolute inset-0 bg-black/40" onClick={closeMobileActionSheet} />
          <div className="absolute bottom-0 left-0 right-0 bg-white p-4 rounded-t-2xl shadow-2xl dark:bg-gray-800 transition-transform">
            ...
            <p className="text-xs text-gray-500 mb-4">{selectedMobileRow?.jobCode} - {selectedMobileRow?.jobName}</p>
            <div className="space-y-2">
              <button onClick={() => handleEdit(selectedMobileRow)} className="w-full py-3 text-sm font-medium bg-blue-50 text-blue-700 rounded-lg flex items-center justify-center gap-2">
                <FontAwesomeIcon icon={faEdit} /> Edit
              </button>
              <button onClick={() => { closeMobileActionSheet(); handleDelete(selectedMobileRow); }} className="w-full py-3 text-sm font-medium bg-red-50 text-red-600 rounded-lg flex items-center justify-center gap-2">
                <FontAwesomeIcon icon={faTrashAlt} /> Delete
              </button>
              <button onClick={closeMobileActionSheet} className="w-full py-3 text-sm font-medium text-gray-500 rounded-lg">Cancel</button>
            </div>
          </div>
        </div>
      )}
