  ...
  import VETypeCodes from "@/NAYSA Cloud/Master Data/VEMasterData/ReferenceCodes/VETypeCodes";
  ...
  const VEMast_ReferenceCodeTab = forwardRef(({
    ...
    const refTabs = useMemo(
      () => [
        ...
        { id: "type", label: "Vehicle Type Codes", icon: faTruckPickup },
        { id: "partClass", label: "Vehicle Part Class Codes", icon: faTruckPickup },
      ],
      []
    );
    ...
    useImperativeHandle(
      ...
      () => ({
        add: () => {
          ...
          if (activeRefTab === "type") typeRef.current?.add?.();
          if (activeRefTab === "partClass") partclassRef.current?.add?.();
        ...
        save: () => {
          ...
          if (activeRefTab === "type") typeRef.current?.save?.();
          if (activeRefTab === "partClass") partclassRef.current?.add?.();
        ...
        reset: () => {
          ...
          if (activeRefTab === "type") typeRef.current?.reset?.();
          if (activeRefTab === "partClass") partclassRef.current?.add?.();
    ...
    const renderRight = () => {
      switch (activeRefTab) {
        ...
        case "type":
          return <VETypeCodes ref={typeRef} onStateChange={childStateChange} {...permissionProps} />;
        case "partClass":
          return <VEPartClass ref={partclassRef} onStateChange={childStateChange} {...permissionProps} />;
        default:
          return null;
      }
    };
