...
const MSClassificationCodes = forwardRef(
  (
    {
      onStateChange,
      isReadOnly = false,
    ...
    const checkDuplicate = useCallback(
      async (code, categCode) => {
        ...
        try {
          const res = await apiClient.post("/checkDuplicateMSClass", {
            json_data: {
              code: c,
              categCode: category,
            },
          ...
          return classifications.some(
            (row) =>
              String(row.code || "").trim().toUpperCase() === c.toUpperCase() &&
              String(row.categCode || "").trim().toUpperCase() === category.toUpperCase()
          );
        }
      },
      [classifications]
      ...
      <div className="flex flex-col h-full gap-3 w-full relative">
        ...
        <div className="flex-1 bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden min-h-[300px] flex flex-col">
          <SearchGlobalReferenceTable
            ...
            docType="MS Classification Codes"
            itemsPerPage={50}
            onRowDoubleClick={
...
MSClassificationCodes.displayName =
  "MSClassificationCodes";

export default MSClassificationCodes;
