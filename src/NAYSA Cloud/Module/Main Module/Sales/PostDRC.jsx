...
2. System loads open Delivery Receipt Consignment records from endpoint: postingDRC.
3. System loads table column setup from hs_colconfig using endpoint postingDRC.
4. User selects one or more DR records.
5. User clicks Okay/Post.
6. System calls global posting function with doc code DRC.
7. Posting procedure finalizes selected records, transfers picked FG stock to
   the destination warehouse/location, and finalizes FG picking allocation.
================================================================================
*/

const PostDRC = ({ isOpen, onClose, userCode }) => {
  ...
  */
    ...
    const fetchPostingData = async () => {
      ...
      try {
        ...
        useSwalValidationAlert({
          ...
          message: "Unable to load Delivery Receipt Consignment records for posting.",
        });
  ...
  |--------------------------------------------------------------------------
  | useHandlePostTran will call posting API/procedure using doc code DRC.
  */
  ...
  */
  const pickDocAndBranch = (row) => {
    ...
    const docNo = row.drcNo || row.docNo || row.DRCNo || row.DRC_NO || null;
  ...
  const handleViewDocument = (row) => {
    ...
    if (!docNo || !branchCode) {
      useSwalValidationAlert({
        icon: "warning",
        title: "Missing Keys",
        message: "Cannot determine DRC No. or Branch Code.",
      });
      return;
    }

    const DR_VIEW_URL = "/tran/DRC";
    const url =
      `${window.location.origin}${DR_VIEW_URL}` +
      `?drcNo=${encodeURIComponent(docNo)}` +
    ...
    <>
      {modalReady && (
        <GlobalGLPostingModalv1
          ...
          title="Finalize Delivery Receipt Consignment"
