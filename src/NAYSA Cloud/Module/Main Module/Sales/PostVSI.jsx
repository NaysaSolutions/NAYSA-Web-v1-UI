...
const PostVSI = ({ isOpen, onClose, userCode }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error("Error fetching VSI posting data:", error);
        useSwalValidationAlert({
          icon: "error",
          title: "Posting Load Error",
          message: "Unable to load Vehicle Sales Invoice records for posting.",
        });
        onClose?.();
      } finally {
  ...
  const handlePost = async (selectedData, password) => {
    await useHandlePostTran(selectedData, password, "VSI", userCode, setLoading, onClose);
  };
  ...
  const handleViewDocument = (row) => {
    ...
    window.open(
      `${window.location.origin}/page/VSI?vsiNo=${encodeURIComponent(docNo)}&branchCode=${encodeURIComponent(branchCode)}&viewDocument=true`,
      "_blank",
      "noopener,noreferrer"
    );
  };
  ...
  return <>
    {ready && <GlobalGLPostingModalv1
      data={data}
      colConfigData={columns}
      title="Finalize Vehicle Sales Invoice"
      btnCaption="Okay"
      onClose={onClose}
      onPost={handlePost}
