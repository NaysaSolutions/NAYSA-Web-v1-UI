...
const DRC = () => {
  ...
  const [state, setState] = useState({
    ...
    openSODRC_Col_Summary: [],
    openSODRC_Col_Detail: [],
  ...
  const isHeaderDrcStatusEditable = !!String(documentID || "").trim() && !isFormDisabled;
  const canUsePickingControls = !isViewDocumentUrl && !isPosted && !isCancelled;
  const filteredHeaderDrcStatusOptions = drcStatusOptions || [];
  const selectedDrcTranTypeOption = (drcTranTypeOptions || []).find(
    (option) => option.DROPDOWN_CODE === (drcTranType || "DRC01")
  );
  const selectedDrcTranTypeText = `${drcTranType || "DRC01"} ${selectedDrcTranTypeOption?.DROPDOWN_NAME || ""}`.toUpperCase();
  const isRegularDrType = (drcTranType || "DRC01") === "DRC01" || selectedDrcTranTypeText.includes("REGULAR");
  ...
  const [totals, setTotals] = useState({
    totalDrcQuantity: formatNumber(0, quantityDecimals),
  ...
  const detailColumnDefs = [
    ...
    { key: "drcStat", label: "Picking Status", width: 130 },
    ...
    { key: "drcQuantity", label: "Consignment Quantity", width: 120 },
  ...
  const updateTotalsDisplay = (totalDrQty, totalPickedQty) => {
    setTotals({
      totalDrcQuantity: formatNumber(totalDrQty, quantityDecimals),
...
useEffect(() => {
    ...
    const filteredTypes = getAllDropDown("DRCTRAN_TYPE", docType) || [];
    const defaultDrType =
      filteredTypes.find((type) => type.DROPDOWN_CODE === "DRC01")?.DROPDOWN_CODE ||
    ...
    updateState({
      drcTranTypeOptions: filteredTypes,
      ...
      drcStatus: mapHeaderDrcStatus(state.drcStatus),
    });
}, [docType, refsLoaded]);
...
const fetchTranData = async (documentNo, branchCode,direction='') => {
  ...
  try { // Assuming 'drcNo' is the correct field for DR document number
    const data = await useFetchTranData(documentNo, branchCode, docType, "drcNo", direction);
    ...
    const retrievedDetailRows = (data.dt1 || []).map(item => ({
      ...
      drcQuantity: formatNumber(item.drcQuantity ?? 0, quantityDecimals),
    }));
    ...
    updateState({
      documentStatus: data.drcStatus,
      status: data.docStatus,
      noReprints:data.noReprints,
      documentID: data.drcId,
      documentNo: data.drcNo,
      documentDate: useformatToDatev2(data.drcDate),
      ...
      destWhouseCode: data.destWhouseCode || "",
      destWhouseName: data.destWhouseName || "",
      destLocCode: data.destLocCode || "",
      destLocName: data.destLocName || "",
      refDocNo1: data.refDrcNo1,
      refDocNo2: data.refDrcNo2,
      remarks: data.remarks,
      drcStatus: String(data.drcStatus || "O").toUpperCase() === "OPEN" ? "O" : String(data.drcStatus || "O").toUpperCase() === "CANCELLED" ? "X" : String(data.drcStatus || "O").toUpperCase() === "CLOSED" ? "C" : String(data.drcStatus || "O"), 
      originalDocStatus: data.drcStatus,
  ...
  } catch (error) { // Changed soNo to drcNo
...
const handleActivityOption = async (action, options = {}) => {
  ...
  if (action === "Upsert") {
    ...
    const isValid = await useSwalvalidateRequiredFields(requiredFields, "Delivery Receipt Consignment");
    if (!isValid) return;

    if (
      String(whseCode || "").trim().toUpperCase() === String(destWhouseCode || "").trim().toUpperCase() &&
      String(locCode || "").trim().toUpperCase() === String(destLocCode || "").trim().toUpperCase()
    ) {
      useSwalErrorAlert(
        "Delivery Receipt Consignment",
        "Destination warehouse and location must be different from the source warehouse and location."
      );
      return;
    }
  }
    ...
    try {
      ...
      const buildDrData = (glRows = []) => ({
        ...
        dt1: workingDetailRows.map((row, index) => ({
          ...
          drcQuantity: parseFormattedNumber(row.drcQuantity || 0),
      ...
      if (action === "Upsert") {
        ...
        */
        ...
        if (response) {
          const responseDocNo = response.data[0].drcNo;
          const responseDocId = response.data[0].drcId;
        ...
        updateState({
          documentNo: response?.data?.[0]?.drcNo || "",
          documentID: response?.data?.[0]?.drcId || "",
          isDocNoDisabled: true,
          isFetchDisabled: true,
        });
      }
      ...
      useSwalErrorAlert("Delivery Receipt Consignment", getApiErrorMessage(error));
    } finally {
      updateState({ isLoading: false, showSpinner: false });
    }
  }
  ...
  const createDRDetailRow = (overrides = {}) => ({
      ...
      freeItem: "", 
      drcQuantity: Number(0).toFixed(quantityDecimals),
  ...
  const getFilteredDuplicateFreeItems = (records = [], currentRowIndex = null) => {
    ...
    if (skippedItemCodes.length > 0) {
      useSwalErrorAlert(
        ...
        `These item(s) already exist in DRC Detail: ${[...new Set(skippedItemCodes)].join(", ")}`
      );
    }
  ...
  const mapOpenSORecordToDetailRow = (item = {}) => {
    ...
    return createDRDetailRow({
      ...
      deliveryDate: useformatToDatev2(item?.deliveryDate),
      drcQuantity: formatNumber(item?.drcQuantity ?? soBalanceValue ?? 0, quantityDecimals),
      quantityPicked: formatNumber(item?.quantityPicked ?? 0, quantityDecimals),
      freeItem: item?.freeItem || "",
    });
  };
...
const cancelPickingAllocationForDeletedRow = async (row) => {
  ...
  try {
    ...
    useSwalErrorAlert("Delete DRC Detail", getApiErrorMessage(error));
    return false;
  } finally {
    updateState({ isLoading: false, showSpinner: false });
  }
...
const handlePrint = async () => {
 if (!detailRows || detailRows.length === 0) {
  ...
  const buildPickingBasePayload = (row, index) => ({
    ...
    lineNo: index + 1,
    itemCode: row?.itemCode || "",
    requestedQty: parseFormattedNumber(row?.drcQuantity || 0) || 0,
    userCode: userCode || currentUserRow?.userCode || "",
  });
  ...
  const handleOpenSalesOrderLookup = async (overrides = {}) => {
    ...
    try {
      ...
      const endpoint = "getSODRC_OpenSummary";
      let response;
      try {
      ...
      const detailColumns = await selectedHSColConfig("getSODRC_OpenDetail");

      updateState({
        openSODRC_Data_Summary: soRows,
        openSODRC_Col_Summary: summaryColumns,
        openSODRC_Col_Detail: detailColumns,
...
const handleOpenItemPickingModal = async (index) => {
  ...
  const requestedQty = parseFormattedNumber(row?.drcQuantity || 0) || 0;
  ...
  if (requestedQty <= 0) {
    useSwalErrorAlert("Item Picking", "Consignment Quantity must be greater than zero before opening the picking allocation.");
    return;
  }
...
const handleConfirmItemPicking = async (payload) => {
  ...
  try {
    ...
    const requestedQty = parseFormattedNumber(currentRow?.drcQuantity || 0) || 0;
    ...
    const drcQuantityValue = parseFormattedNumber(currentRow?.drcQuantity || 0) || 0;

    if (totalPicked > drcQuantityValue) {
      useSwalErrorAlert("Item Picking", "Picked quantity cannot be more than requested quantity.");
      return;
    }
    ...
    updatedRows[itemPickingRowIndex] = {
      ...
      drcStat:
        totalPicked <= 0
          ? "F"
          : drcQuantityValue > 0 && totalPicked >= drcQuantityValue
            ? "P"
            : "T",
...
const handleHeaderDrcStatusChange = async (value) => {
  if (value === "X") {
    await handleCancel();
    return;
  }

  if (String(drcStatus || "").toUpperCase() === "O" && value === "C") {
    const result = await useSwalProceedConfirm(
      "Confirm Close DR",
      "Do you want to change DRC Status from Open to Closed?",
      "Yes"
    );
...
useEffect(() => {
  ...
  const docNo = params.get("drcNo") || params.get("soNo");
...
const handleSaveAndPrint = async (documentID) => {
  ...
  const updateTotals = (rows) => { // This function might not be needed if gross/net/discount are removed
    ...
    rows.forEach(row => {
      totalDrQty += parseFormattedNumber(row.drcQuantity || 0) || 0;
...
const handleCloseWarehouseLookup = (row) => {
  if (row) {
    const warehouseCode = row.whCode || row.whouseCode || "";
    const warehouseName = row.whName || row.whouseName || "";
    updateState(modalContext === "destinationWarehouse" ? {
      destWhouseCode: warehouseCode,
      destWhouseName: warehouseName,
      destLocCode: "",
      destLocName: "",
    } : {
      whseCode: warehouseCode,
      whseName: warehouseName,
      locCode: "",
      locName: "",
    });
  }

  updateState({ showWhseModal: false, modalContext: "" });
};

const handleCloseLocationLookup = (row) => {
  if (row) {
    updateState(modalContext === "destinationLocation" ? {
      destLocCode: row.locCode || "",
      destLocName: row.locName || "",
    } : {
      locCode: row.locCode || "",
      locName: row.locName || "",
    });
  ...
  updateState({ showLocModal: false, modalContext: "" });
};
...
const validateSOQuantity = (index, inputValue) => {
  const row = detailRowsRef.current[index];
  const drQty = parseFormattedNumber(row?.drcQuantity || 0) || 0;
  ...
  if (drQty > 0 && rowStatus === "O" && soQty < drQty) {
    ...
    useSwalErrorAlert("Invalid Quantity", "SO Quantity must be greater than or equal to Consignment Quantity.");
...
const handleSODetailRowChange = (index, field, value) => {
  ...
  if (field === "deliveryDate") {
    ...
    const drcDateValue = parseDateValue(documentDate);

    if (nextDate && drcDateValue) {
      nextDate.setHours(0, 0, 0, 0);
      drcDateValue.setHours(0, 0, 0, 0);

      if (nextDate < drcDateValue) {
        useSwalErrorAlert("Invalid Delivery Date", "Delivery Date cannot be earlier than DRC Date.");
        return;
      }
    }
  }
  ...
  const zeroValueByField = (targetField) => {
    if (targetField === "soQuantity" || targetField === "drcQuantity") {
...
const renderDRDetailCell = (columnKey, row, index) => {
  ...
  const isRowWithDR = (parseFormattedNumber(row.drcQuantity || 0) || 0) > 0;
  ...
  const detailColumnRenderers = {
    ...
    drcStat: () => <td key={columnKey} className="global-tran-td-ui" style={style}><select id={`drcStat-${index}`} className="w-full global-tran-td-inputclass-ui text-left" value={row.drcStat || "F"} disabled={isFormDisabled || !canEditPickingStatus} onChange={(e) => handleSODetailRowChange(index, "drcStat", e.target.value)} onKeyDown={(e) => { if (e.key !== "Enter" || isFormDisabled || !canEditPickingStatus) return; e.preventDefault(); focusNextDetailCell("drcStat"); }}><option value="F">For Picking</option>{canEditPickingStatus ? <option value="X">Cancelled</option> : <><option value="T">Partially Picked</option><option value="P">Picked</option><option value="X">Cancelled</option></>}</select></td>,
    ...
    drcQuantity: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{numericInput(columnKey, { decimals: quantityDecimals, regex: new RegExp(`^\\d*\\.?\\d{0,${quantityDecimals}}$`) })}</td>, 
    quantityPicked: () => (
        ...
        <div className="flex items-center gap-1">
          ...
          {canUsePickingControls && (
            <button
              ...
              disabled={!row?.groupId || !row?.itemCode || (parseFormattedNumber(row?.drcQuantity || 0) || 0) <= 0}
...
<div className="global-tran-main-div-ui"> 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 rounded-lg relative" id="dr_hd">
          ...
          <div className="global-tran-textbox-group-div-ui">
            ...
            <FieldRenderer
              ...
              onLookup={() => updateState({ showAllTranDocNo: true })} // Changed soNo to drcNo
              onKeyDown={(e) => { 
                if (e.key === "F1") {
                  e.preventDefault();
                  updateState({ showAllTranDocNo: true });
            ...
            <FieldRenderer
              ...
              label="DRC Type"
              type="select"
              value={drcTranType || "DRC01"}
              disabled={isFormDisabled}
              onChange={(val) => updateState({ drcTranType: val })}
              options={drcTranTypeOptions.map((t) => ({
          ...
          <div className="global-tran-textbox-group-div-ui">
            <FieldRenderer
              ...
              editableLookup
              onLookup={() => updateState({ showWhseModal: true, modalContext: "sourceWarehouse" })}
              onClear={() =>
                updateState({
                  whseCode: "",
                  whseName: "",
            ...
            <FieldRenderer
              ...
              onLookup={() =>
                !isFormDisabled &&
                whseCode &&
                updateState({ showLocModal: true, modalContext: "sourceLocation" })
              }
            ...
            <FieldRenderer
              ...
              required
              type="lookup"
              value={destWhouseName || ""}
              disabled={isFormDisabled}
              readOnly
              editableLookup
              onLookup={() => updateState({ showWhseModal: true, modalContext: "destinationWarehouse" })}
              onClear={() => updateState({
                destWhouseCode: "",
                destWhouseName: "",
                destLocCode: "",
            ...
            <FieldRenderer
              ...
              type="lookup"
              value={destLocName || ""}
              disabled={isFormDisabled || !destWhouseCode}
              readOnly
              lookupDisabled={!destWhouseCode}
              editableLookup
              onLookup={() => destWhouseCode && updateState({ showLocModal: true, modalContext: "destinationLocation" })}
              onClear={() => updateState({ destLocCode: "", destLocName: "" })}
            />
          </div>
          ...
          <div className="global-tran-textbox-group-div-ui">
            ...
            <FieldRenderer
              ...
              type="select"
              value={drcStatus || ""}
              disabled={!isHeaderDrcStatusEditable}
              onChange={handleHeaderDrcStatusChange}
              options={filteredHeaderDrcStatusOptions.map((t) => ({
                label: t.DROPDOWN_NAME,
                value: t.DROPDOWN_CODE,
              }))}
            />
      ...
      <div className="global-tran-table-main-sub-div-ui">
        <table className="min-w-full border-separate border-spacing-0 [&_th]:border-b [&_th]:border-slate-200 [&_td]:border-t-0 [&_td]:border-l-0 [&_td]:border-r [&_td]:border-b [&_td]:border-slate-200 [&_tr>td:first-child]:border-l">
          ...
          <tbody className="relative">{sortedDetailRows.map(({ row, originalIndex }) => {
            ...
            const pickedQty = parseFormattedNumber(row.quantityPicked || 0) || 0;
            const canDeleteRow = pickedQty > 0;
    ...
    <div className="global-tran-tab-footer-button-div-ui">
      ...
      >
        ...
        <div className="global-tran-tab-footer-total-label-ui">Total Consignment Quantity:</div>
        <div className="global-tran-tab-footer-total-value-ui">{totals.totalDrcQuantity}</div>
      ...
      {/* General Ledger */}
      {false && canViewCostAmount && hasPickedQuantity && (
    ...
    {showLocModal && (
      <LocationLookupModal
        ...
        filter={"ByWH" + (modalContext === "destinationLocation" ? destWhouseCode : whseCode)}
      />
    )}
    ...
    {showOpenSOModal && (
      <GlobalCombinedLookup
        ...
        summaryColumns={openSODRC_Col_Summary}
        detailColumns={openSODRC_Col_Detail}
        summaryData={openSODRC_Data_Summary}
    ...
    {showItemPickingModal && selectedPickingRow && (
      <SearchGlobalItemPickingModal
        ...
        transaction={{
          sourceDocType: "DRC",
          sourceDocTypeName: "Delivery Receipt Consignment",
          ...
          requestedQty: parseFormattedNumber(selectedPickingRow?.drcQuantity || 0) || 0,
        }}
    ...
    {showAllTranDocNo && (
      <AllTranDocNo
        ...
        params={{branchCode,branchName,docType,documentTitle,fieldNo : "drcNo"}}
  ...
  <AllTranHistory
    ...
    endpoint="/getDRCHistory"
    cacheKey={`DR:${state.branchCode || ""}`}
    activeTabKey="DRC_Summary"
