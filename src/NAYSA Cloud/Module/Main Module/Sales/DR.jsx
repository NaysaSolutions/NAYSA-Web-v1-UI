...
//     { key: "drStat", label: "DR Status", width: 110 },
...
//           lnNo: String(index + 1),
//           drStat: row.drStat || "O", // Changed soStat to drStat
//           soNo: row.soNo || "",
...
//       lnNo: "",
//       drStat: "O", // Changed soStat to drStat
//       soNo: "",
//       itemCode: "",
//       itemName: "",
//       itemSpecs: "",
...
//   const handleOpenSalesOrderLookup = async () => {
//     const fieldsToCheck = {
//       "Header : Bill To Customer Code": shipToCode,
...
//           payload: { custCode: shipToCode, shipToCode, branchCode },
...
// const renderDRDetailCell = (columnKey, row, index, drStatusOptions) => {
...
//     drStat: () => { const isStatusLocked = !documentID || ["X", "C"].includes(row.drStat || "O"); return <td key={columnKey} className="global-tran-td-ui" style={style}><select id={`drStat-${index}`} className="w-full global-tran-td-inputclass-ui text-left" value={row.drStat || "O"} disabled={isFormDisabled || isStatusLocked} onChange={(e) => handleSODetailRowChange(index, "drStat", e.target.value)} onKeyDown={(e) => { if (e.key !== "Enter" || isFormDisabled || isStatusLocked) return; e.preventDefault(); focusNextDetailCell("drStat"); }}>{drStatusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></td>; },
...
//     deliveryDate: () => <td key={columnKey} className="global-tran-td-ui" style={style}><DateFormatInput value={row.deliveryDate || ""} disabled={isFormDisabled || isRowWithDR} onChange={(value) => handleSODetailRowChange(index, "deliveryDate", value)} className="w-full h-7 text-xs bg-transparent focus:outline-none focus:ring-0" /></td>,
...
//     quantityPicked: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{numericInput(columnKey, { decimals: quantityDecimals, regex: new RegExp(`^\\d*\\.?\\d{0,${quantityDecimals}}$`) })}</td>,
//     siNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput(columnKey, { readOnly: true })}</td>,
//     siDate: () => <td key={columnKey} className="global-tran-td-ui" style={style}><DateFormatInput value={row.siDate || ""} disabled className="w-full h-7 text-xs bg-transparent focus:outline-none focus:ring-0" /></td>,
//   };
...
//               readOnly
//               lookupDisabled={isFetchDisabled || !whseCode}
//               onLookup={() =>
//                 !isFormDisabled &&
//                 !isFetchDisabled &&
//                 whseCode &&
...
//             const canDeleteRow = !hasDrQuantity && (row.drStat || "O") === "O"; // Changed soStat to drStat
//             const drStatusOptions = hasDrQuantity
//               ? [
//                   { value: "O", label: "Open" },
...
//               {orderedDetailColumns.map((column) => 
//                 renderDRDetailCell(column.key, row, originalIndex, drStatusOptions) // Changed renderSODetailCell to renderDRDetailCell
//               )}
