...
//       whouseCode: data.whouseCode || "",
//       whouseName: data.whouseName || "",
...
//                 whouseCode: selectedWarehouse?.whouseCode || finalWhseCode || "",
//                 whouseName: selectedWarehouse?.whouseName || "",
//                 locCode: "",
//                 locName: "",
//             });
...
// UI
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faMagnifyingGlass, faPlus, faMinus, faTrashAlt, faSpinner,faSearch } from "@fortawesome/free-solid-svg-icons";
...
const VDR = () => {
  ...
  // Company defaults - moved here to ensure they are defined before use in useState
  // A VDR always represents one whole vehicle, so quantity has no decimals.
  ...
  const { resetFlag } = useReset();
  const [focusedCell, setFocusedCell] = useState(null); // { index: number, field: string }
  const docType = docTypes.VDR || "VDR";
  const hsDoc = getAllTopHSDocRow(docType);
  const pdfLink = docTypePDFGuide[docType];
  const videoLink = docTypeVideoGuide[docType];
  const documentTitle = (hsDoc?.docName || '') + ' Transaction';
  ...
  useEffect(() => {
    if (documentID || branchCode || !currentUserRow?.branchCode) return;
  ...
  }, [branchCode, currentUserRow, documentID, userCode]);
  ...
  const detailColumnDefs = [
    ...
    { key: "ln", label: "LN", width: 56 },
    { key: "drStat", label: "Picking Status", width: 130 },
    { key: "soNo", label: "VSO No.", width: 140 },
    { key: "csNo", label: "CS / Chassis No.", width: 160 },
    { key: "itemCode", label: "Item Code", width: 140 },
    { key: "itemName", label: "Item Name", width: 240 },
    { key: "itemSpecs", label: "Specification", width: 240 },
    { key: "uomCode", label: "UOM", width: 100 },
    { key: "drQuantity", label: "Quantity", width: 100 },
    { key: "quantityPicked", label: "Quantity Picked", width: 130 },
    { key: "itemAmount", label: "Item Amount", width: 130 },
    { key: "deliveryDate", label: "Delivery Date", width: 130 },
    { key: "siNo", label: "SI No.", width: 140 },
  ...
  const hasPickedQuantity = getTotalQuantityPicked(detailRows) > 0;
  const isPickingStatusPicked =
    (detailRows?.length || 0) > 0 &&
    detailRows.every((row) => String(row?.drStat || "").toUpperCase() === "P");
...
useEffect(() => {
  ...
  const handleReset = () => {
    ...
    const loadCompanyData = async () => {
        ...
        try {
          const hdtblcol_result = await useFieldLenghtCheck(
            "vdr_hd,vdr_dt1,vdr_dt2,vdr_dt3",
          );
...
const fetchTranData = async (documentNo, branchCode,direction='') => {
  ...
  try { // Assuming 'drNo' is the correct field for DR document number
    const data = await useFetchTranData(documentNo, branchCode, docType, "vdrNo", direction);
    ...
    const retrievedDetailRows = (data.dt1 || []).map(item => ({
      ...
      soId: item.vsoId || "",
      veId: item.veId || "",
      soBalance: formatNumber(item.vsoBal ?? 0, quantityDecimals),
      drQuantity: formatNumber(item.quantity ?? 0, quantityDecimals),
      quantityPicked: formatNumber(item.qtyPicked ?? 0, quantityDecimals),
      itemAmount: formatNumber(item.amount ?? 0),
      freeItem: item.freeItem || "",
      deliveryDate: useformatToDatev2(item.vsoDeldate),
      siNo: item.vsiNo || "",
      siDate: useformatToDatev2(item.vsiDate),
    }));
    ...
    updateState({
      documentStatus: data.vdrStatus || "O",
      status: data.docStatus,
      noReprints:data.noReprints,
      documentID: data.vdrId,
      documentNo: data.vdrNo,
      documentDate: useformatToDatev2(data.vdrDate),
      branchCode: data.branchCode,
      branchName:data.branchName,
      drTranType: data.soDoctype || "VDR01",
      shipToCode: data.custCode,
      shipToName: data.custName,
      shipToAddress: [data.delivAdd1, data.delivAdd2, data.delivAdd3].filter(Boolean).join(" "),
      whouseCode: data.whouseCode || "",
      whouseName: data.whouseName || "",
      locCode: data.locCode || "",
      locName: data.locName || "",
      refDocNo1: data.refdrNo || "",
      refDocNo2: "",
      remarks: data.particular || "",
      drStatus: data.vdrStatus || "O",
      originalDocStatus: data.vdrStatus || "O",
      detailRows: retrievedDetailRows,
      detailRowsGL: formattedGLRows,
      isDocNoDisabled: true,
      isFetchDisabled: true,
...
const handleActivityOption = async (action) => {
  ...
  if (documentStatus === "O" || action === "GenerateGL") {
    ...
    try {
      ...
      const selectedVsoId = detailRows?.[0]?.soId || "";
      const selectedVsoNo = detailRows?.[0]?.soNo || "";
      const effectiveBranchCode = String(
        branchCode || detailRows?.[0]?.branchCode || currentUserRow?.branchCode || ""
      ).trim();
      const effectiveDocumentDate = documentDate || useGetCurrentDayV2();
      const buildDrData = (glRows = []) => ({
        branchCode: effectiveBranchCode,
        vdrNo: documentNo || "",
        vdrId: documentID || "",
        vdrDate: effectiveDocumentDate,
        vsoId: selectedVsoId,
        vsoNo: selectedVsoNo,
        soDoctype: drTranType || "VDR01",
        custCode: shipToCode,
        custName: shipToName,
        shipToCode,
        shipToName,
        delivAdd1: shipToAddress,
        refdrNo: refDocNo1,
        whouseCode: whouseCode,
        locCode,
        particular: remarks || "",
        ...
        dt1: detailRows.map((row, index) => ({
          lnNo: String(index + 1),
          veId: row.veId || "",
          vdrStat: row.drStat || "O",
          itemCode: row.itemCode || "",
          itemName: row.itemName || "",
          uomCode: row.uomCode || "",
          vsoBal: parseFormattedNumber(row.soBalance || 0),
          quantity: parseFormattedNumber(row.drQuantity || 0),
          qtyPicked: parseFormattedNumber(row.quantityPicked || 0),
          amount: parseFormattedNumber(row.itemAmount || 0),
          vsoNo: row.soNo || "",
          vsoDeldate: row.deliveryDate || null,
          vsiNo: row.siNo || "",
          vsiDate: row.siDate || null,
          itemSpecs: row.itemSpecs || "",
          groupId: row.groupId || "",
          vsoId: row.soId || selectedVsoId,
        })),
        dt2: glRows.map((entry, index) => ({
          recNo: String(index + 1),
          acctCode: entry.acctCode || "",
        ...
        */
        if (totalPickedQuantity <= 0) {
          ...
          if (finalDetailRowsGL.length === 0) {
            const generatedRows = await useGenerateGLEntries(
              docType,
              buildDrData([])
            );
        ...
        if (response) {
          const responseDocNo = response.data[0].vdrNo;
          const responseDocId = response.data[0].vdrId;
        ...
        updateState({
          documentNo: response?.data?.[0]?.vdrNo || "",
          documentID: response?.data?.[0]?.vdrId || "",
          isDocNoDisabled: true,
          isFetchDisabled: true,
        });
      }
...
useEffect(() => {
  ...
  const resolveOpenSOGroupId = (item = {}) => item?.groupId || item?.vsoId || item?.soId || "";

  const normalizeOpenSOLookupRow = (item = {}) => ({
    ...item,
    groupId: resolveOpenSOGroupId(item),
    soId: item?.soId || item?.vsoId || "",
    soNo: item?.soNo || item?.vsoNo || "",
    deliveryDate: item?.deliveryDate || "",
  });

  const mapOpenSORecordToDetailRow = (item = {}) => {
    const soBalanceValue = item?.soBalance ?? item?.vsoBalance ?? 0;
  ...
  const handleInsertSelectedOpenSO = async (payload) => {
    ...
    const rowsToInsert = selectedRecords.slice(0, 1).map((record) => ({
      ...mapOpenSORecordToDetailRow(record),
      soNo: record?.vsoNo || record?.soNo || "",
      csNo: record?.csNo || "",
      soId: record?.vsoId || record?.soId || "",
      veId: record?.veId || "",
      quantityPicked: formatNumber(record?.quantity ?? record?.drQuantity ?? record?.vsoBalance ?? 1, quantityDecimals),
      drQuantity: formatNumber(record?.quantity ?? record?.drQuantity ?? record?.vsoBalance ?? 1, quantityDecimals),
    }));
    const selectedShipToAddress = getOpenSOShipToAddress(headerSourceRecords);
    const selectedHeader = headerSourceRecords[0] || selectedRecords[0] || {};
    const nextRemarks = appendMissingRemarks(
      remarks,
      getUniqueOpenSORemarks(headerSourceRecords)
    );

    detailRowsRef.current = rowsToInsert;
    updateState({ detailRows: normalizeDetailLineNumbers(rowsToInsert) });
    updateTotals(rowsToInsert);
    setTopTab("details");

    updateState({
      branchCode: selectedHeader.branchCode || branchCode || currentUserRow?.branchCode || "",
      branchName: selectedHeader.branchName || branchName || currentUserRow?.branchName || "",
      shipToCode: selectedHeader.custCode || "",
      shipToName: selectedHeader.custName || "",
      ...(selectedShipToAddress ? { shipToAddress: selectedShipToAddress } : {}),
      whouseCode: selectedHeader.whouseCode || whouseCode || "",
      remarks: nextRemarks,
      showOpenSOModal: false,
      openSODR_Data_Summary: [],
      openSODR_Col_Summary: [],
 ...
 if (!detailRows || detailRows.length === 0) {
  ...
  const getPickingResultRow = (result, row, index) => {
    ...
    return (
      ...
      resultRows.find((resultRow) => {
        const resultItemCode = String(resultRow.itemCode || "");
        return itemCode && resultItemCode === itemCode;
      }) ||
      resultRows[0]
    );
  ...
  const handleOpenSalesOrderLookup = async (overrides = {}) => {
    ...
    try {
      ...
      if (soRows.length === 0) {
        useSwalErrorAlert(
          "Open Sales Order",
          "There are no eligible open Vehicle Sales Orders for the selected branch."
        );
        return;
      }
...
const handleHistoryRowPick = useCallback(
  async (row) => {
    const selectedVdrNo = row?.vdrNo || row?.docNo;
    const selectedBranchCode = row?.branchCode;
    if (!selectedVdrNo || !selectedBranchCode) return;

    await fetchTranData(selectedVdrNo, selectedBranchCode);
    setTopTab("details");
    cleanUrl(); //
  },
  [fetchTranData, cleanUrl]
...
useEffect(() => {
  const params = new URLSearchParams(location.search);
  const docNo = params.get("vdrNo") || params.get("docNo");
  const branchCode = params.get("branchCode");
...
const handleTranDocNoRetrieval = async (data) => {
    const retrievalBranchCode = data.branchCode || state.branchCode || branchCode;
    await fetchTranData(data.docNo, retrievalBranchCode, data.key);
    updateState({showAllTranDocNo: data.modalClose});
    setTopTab("details");
};
...
const renderDRDetailCell = (columnKey, row, index) => {
  ...
  const detailColumnRenderers = {
    ...
    freeItem: () => <td key={columnKey} className="global-tran-td-ui" style={style}><button type="button" className={`w-full h-7 rounded-full border text-[11px] font-semibold transition-colors ${row.freeItem === "Y" ? "border-blue-500 bg-blue-500/15 text-blue-700" : "border-slate-300 bg-white text-slate-600"} ${isFormDisabled || isRowFromSO ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`} disabled={isFormDisabled || isRowFromSO} onClick={() => handleSODetailRowChange(index, "freeItem", row.freeItem === "Y" ? "" : "Y")}>{row.freeItem === "Y" ? "Yes" : "No"}</button></td>, 
    drQuantity: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{numericInput(columnKey, { decimals: quantityDecimals, regex: /^\d*$/, readOnly: true })}</td>, 
    quantityPicked: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{numericInput(columnKey, { decimals: quantityDecimals, regex: /^\d*$/, readOnly: true })}</td>,
    itemAmount: () => (
      <td key={columnKey} className="global-tran-td-ui" style={style}>
        <input
          type="text"
...
<div className="global-tran-main-div-ui"> 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 rounded-lg relative" id="dr_hd">
          ...
          <div className="global-tran-textbox-group-div-ui">
            ...
            <FieldRenderer
              ...
              label="VSO No."
              type="lookup"
              value={detailRows?.[0]?.soNo || ""}
              disabled={isFormDisabled || Boolean(documentID)}
              readOnly
              onLookup={handleOpenSalesOrderLookup}
            />
          ...
          <div className="flex justify-end">
            {!isFormDisabled && (
              <button
                type="button"
                className="global-tran-button-generateGL"
                disabled={isGeneratingGL || !isPickingStatusPicked}
                onClick={() => handleActivityOption("GenerateGL")}
              >
                {isGeneratingGL ? <FontAwesomeIcon icon={faSpinner} className="mr-2 animate-spin" /> : null}
                {isGeneratingGL ? "Generating..." : "Generate GL Entries"}
    ...
    {showOpenSOModal && (
      <GlobalLookupModalv1
        isOpen={showOpenSOModal}
        title="Open Vehicle Sales Orders"
        endpoint={openSODR_Col_Summary}
        data={openSODR_Data_Summary}
        btnCaption="Select VSO"
        singleSelect
        preferenceKey="VDR:OpenVSO"
        onClose={handleInsertSelectedOpenSO}
        onCancel={() =>
          updateState({
            showOpenSOModal: false,
    ...
    {showAllTranDocNo && (
      <AllTranDocNo
        isOpen={showAllTranDocNo}
        params={{branchCode,branchName,docType,documentTitle,fieldNo : "vdrNo"}}
        onRetrieve={handleTranDocNoRetrieval}
        onResponse={{documentNo}}
        onSelected={handleTranDocNoSelection}
        onClose={() => updateState({ showAllTranDocNo: false })}
  ...
  <AllTranHistory
    showHeader={false}
    isActive={topTab === "history"}
    endpoint="/getVDRHistory"
    cacheKey={`VDR:${state.branchCode || ""}`}
    activeTabKey="VDR_Summary"
    branchCode={state.branchCode}
    status="All"
    onRowDoubleClick={handleHistoryRowPick}
    historyExportName={`${documentTitle} History`}
