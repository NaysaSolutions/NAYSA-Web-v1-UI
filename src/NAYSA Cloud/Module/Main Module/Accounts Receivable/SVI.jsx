...
const renderSviDetailCell = (columnKey, row, index) => {
  ...
  const detailColumnRenderers = {
    ...
    billName: () => <td key={columnKey} className="global-tran-td-ui" style={style}><div className="relative flex items-center">{detailTextInput("billName", { className: "pr-8", maxLength: useGetFieldLength(tblFieldArray, "billName") })}{!isFormDisabled && <FontAwesomeIcon icon={faSearch} className="absolute right-2 text-blue-600 text-lg cursor-pointer hover:text-blue-900" onClick={() => useSwalHandleOpenSpecsModal(index, detailRows, handleDetailChange, row.billName, "Description", "billName", `Enter description for ${row.billCode || "this item"}...`)} />}</div></td>,
    sviSpecs: () => <td key={columnKey} className="global-tran-td-ui" style={style}><div className="relative flex items-center">{detailTextInput("sviSpecs", { className: "pr-8", maxLength: useGetFieldLength(tblFieldArray, "sviSpecs") })}{!isFormDisabled && <FontAwesomeIcon icon={faSearch} className="absolute right-2 text-blue-600 text-lg cursor-pointer hover:text-blue-900" onClick={() => useSwalHandleOpenSpecsModal(index, detailRows, handleDetailChange, row.sviSpecs, "Specification", "sviSpecs", `Enter specification for ${row.billCode || "this item"}...`)} />}</div></td>,
    ...
    atcName: () => <td key={columnKey} className="global-tran-td-ui" style={style}><input type="text" className="w-full global-tran-td-inputclass-ui" value={row[columnKey] || ""} onChange={(e) => handleDetailChange(index, "atcName", e.target.value)} onDoubleClick={!isFormDisabled ? () => handleDetailChange(index, "atcName", 0, true) : undefined} /></td>,
  };
