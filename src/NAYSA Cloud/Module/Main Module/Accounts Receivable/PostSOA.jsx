...
const PostSOA = ({ isOpen, onClose, userCode }) => {
...
const handlePost = async (selectedData, userPw) => {
  await useHandlePostTran(selectedData,userPw,"SOA",userCode,setLoading,onClose)
}


const pickDocAndBranch = (row) => {
  if (!row) return { docNo: null, branchCode: null };
  const docNo = row.soaNo;
...
const SVI_VIEW_URL = "/page/SOA";
const url =
  `${window.location.origin}${SVI_VIEW_URL}` +
  `?soaNo=${encodeURIComponent(docNo)}` +
