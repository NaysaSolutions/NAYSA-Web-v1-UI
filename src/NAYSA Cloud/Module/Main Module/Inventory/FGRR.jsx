...
const FGRR = (item) => {
  ...
  const fgrrQuantityEditStartRef = useRef({});
...
const [isViewDocument, setIsViewDocument] = useState(false);
  ...
  const fgInvGLMode = resolveGLMode(
    companyInfo?.fginvGLMode,
    companyInfo?.fgInvGLMode,
    companyInfo?.FGINV_GLMODE,
    companyInfo?.fginv_glmode,
    companyInfo?.FGINVGLMODE,
    fgInvGLModeSetting,
  );

  // FGINV_GLMODE rules from hs_option:
  // E = show/enable General Ledger / Entries / DT2.
  // D = hide/disable General Ledger / Entries / DT2.
  const isGeneralLedgerEnabled = fgInvGLMode === "E";
...
  ];
  ...
  const withoutCurrencyRateColumn = (columns = []) => {
    const normalizeColumnText = (value) =>
      String(value || "")
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");

    const isCurrencyRateColumn = (column) => {
      const key = normalizeColumnText(column?.key || column?.field || "");
      const label = normalizeColumnText(
        column?.label || column?.header || column?.caption || "",
      );

      return [key, label].some((value) =>
        ["currrate", "currencyrate", "forexrate", "fxrate"].includes(value),
      );
    };

    return columns
      .filter((column) => !isCurrencyRateColumn(column))
...
  };
  ...
  const {
    getColumnStyle: getFGRRDetailColumnStyle,
    getFrozenColumnStyle: getFGRRDetailFrozenStyle,
    getOrderedColumns: getOrderedFGRRDetailColumns,
    getSortedRows: getSortedFGRRDetailRows,
    clearZeroValueOnFocus: clearFGRRDetailZeroOnFocus,
    focusNextRowInput: focusNextFGRRDetailRowInput,
    renderHeaderContextMenu: renderFGRRDetailHeaderContextMenu,
    renderResizableHeader: renderFGRRDetailHeader,
    setHiddenColumnKeys: setFGRRDetailHiddenColumnKeys,
  } = useResizableTableColumns(fgrrDetailColumnDefs);

  useEffect(() => {
    const transactionTypeColumns =
      String(state.fgrrTranType || "FGRR01").toUpperCase() === "FGRR02"
  ...
  const visibleFGRRDetailColumns = useMemo(
    () => {
      const columns = getOrderedFGRRDetailColumns(fgrrDetailColumnDefs);
    ...
    [getOrderedFGRRDetailColumns, fgrrDetailColumnDefs],
  );

  const getFGRRDetailFallbackWidth = (key) =>
    fgrrDetailColumnDefs.find((column) => column.key === key)?.width || 120;

  const getFGRRDetailCellStyle = (key, fallbackWidth) => ({
    ...getFGRRDetailColumnStyle(key, fallbackWidth),
    ...getFGRRDetailFrozenStyle(key, visibleFGRRDetailColumns, fallbackWidth, {
      isHeader: false,
    }),
  });

  const sortedFGRRDetailRows = useMemo(
    () =>
      getSortedFGRRDetailRows(
    ...
    [getSortedFGRRDetailRows, detailRows],
  );

  const fgrrDetailEnterNextRowZeroClearFields = [
    "rrQty",
    "freeQty",
    "convertedQuantity",
    "unitCost",
  ...
  const fgrrGlColumnDefs = useMemo(
    () => [
      { key: "ln", label: "LN", width: 56 },
  ...
  const {
    getColumnStyle: getFGRRGlColumnStyle,
    getFrozenColumnStyle: getFGRRGlFrozenStyle,
    getOrderedColumns: getOrderedFGRRGlColumns,
    getSortedRows: getSortedFGRRGlRows,
    clearZeroValueOnFocus: clearFGRRGlZeroOnFocus,
    focusNextRowInput: focusNextFGRRGlRowInput,
    renderHeaderContextMenu: renderFGRRGlHeaderContextMenu,
    renderResizableHeader: renderFGRRGlHeader,
  } = useResizableTableColumns(fgrrGlColumnDefs);

  const orderedFGRRGlColumns = useMemo(
    () => {
      const orderedColumns = getOrderedFGRRGlColumns(fgrrGlColumnDefs);
    ...
    [getOrderedFGRRGlColumns, fgrrGlColumnDefs],
  );

  const getFGRRGlFallbackWidth = useCallback(
    (key) => fgrrGlColumnDefs.find((column) => column.key === key)?.width || 120,
    [fgrrGlColumnDefs],
  );

  const getFGRRGlCellStyle = useCallback(
    (key, fallbackWidth) => ({
      ...getFGRRGlColumnStyle(key, fallbackWidth),
      ...getFGRRGlFrozenStyle(key, orderedFGRRGlColumns, fallbackWidth, {
        isHeader: false,
      }),
    }),
    [getFGRRGlColumnStyle, getFGRRGlFrozenStyle, orderedFGRRGlColumns],
  );

  const sortedFGRRGlRows = useMemo(
    () =>
      getSortedFGRRGlRows(
    ...
    [getSortedFGRRGlRows, detailRowsGL],
  );

  const fgrrGlEnterNextRowZeroClearFields = [
    "debit",
    "credit",
    "debitFx1",
    "creditFx1",
  ...
  const generateClientGroupId = () => {
    ...
    return `FGRR-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`.toUpperCase();
  };
...
  };
  ...
  const getCurrencyCode = (row = {}) =>
    getPOField(row, "CurrCode", "CURR_CODE", "currCode", "curr_code", "currency", "Currency");

  const getCurrencyName = (row = {}) =>
    getPOField(row, "CurrName", "CURR_NAME", "currName", "curr_name", "currencyName", "CurrencyName");
...
      currency: row.currCode,
      currrate: row.currRate,
      currencyrate: row.currRate,
      forexrate: row.currRate,
      fxrate: row.currRate,
...
          Array.isArray(colConfigDetail) && colConfigDetail.length > 0
 ? withoutCurrencyRateColumn(colConfigDetail)
 : withoutCurrencyRateColumn(openPOColDetail),
...
  const handleClosePOOpenModal = async (selection) => {
  ...
  try {
    ...
    const selectedCurrCode = normalizeCurrencyCode(
      getCurrencyCode(details?.[0]) || getCurrencyCode(summary) || currCode || "PHP",
    );
    const selectedCurrRow = await useTopCurrencyRow(selectedCurrCode);
    const selectedCurrName =
      normalizeCurrencyCode(getCurrencyCode(selectedCurrRow) || selectedCurrCode) === selectedCurrCode
        ? getCurrencyName(selectedCurrRow) || selectedCurrCode
        : selectedCurrCode;
...
categCode: d.categCode || d.CATEG_CODE || d.categ_code || "",
        ...
        convertedQuantity: formatNumber(
          qtyBalance / (parseFormattedNumber(d.conversionFactor) || 1),
          decQty,
        ),
        uomCode2: d.uomCode2 || "",
  ...
  6
    ...
    updateState({
      ...
      currRate: formatNumber(selectedCurrRate || 1, 6),
...
  const handleCloseVatLookup = (vat) => {
    ...
    row = recalcFGRRRowWithCurrencyRate(row);
...
          hs.glCurrDefault ?? hs.GLCURR_DEFAULT ?? state.currCode ?? "PHP";
        ...
        const loadedFGInvGLMode = resolveGLMode(
          hs.fginvGLMode,
          hs.fgInvGLMode,
          hs.FGINV_GLMODE,
          hs.fginv_glmode,
        ...
        setFgInvGLModeSetting(loadedFGInvGLMode);
...
          updateState({
            currName:
              normalizeCurrencyCode(getCurrencyCode(currRow) || transactionCurr) === transactionCurr
                ? getCurrencyName(currRow) || resetCurrency.name || transactionCurr
                : resetCurrency.name || transactionCurr,
...
  const loadDocDropDown = async () => {
    const [poData, fgrrData] = await Promise.all([
      useTopDocDropDown(docType, "POTRAN_TYPE"),
      useTopDocDropDown(docType, "FGRRTRAN_TYPE"),
    ]);

    updateState({
      poTranTypes: poData || [],
      selectedPoTranType: poData?.[0]?.DROPDOWN_CODE ?? "",
      fgrrTranTypes: fgrrData || [],
      fgrrTranType: fgrrData?.[0]?.DROPDOWN_CODE || "FGRR01",
    });
  };
...
  // ==========================
  ...
  const fetchTranData = async (rrNo, branchCode, direction = "") => {
    ...
    try {
      ...
      const parsedWHCode = parsed.whouseCode || "";
      const parsedWHName = parsed.whouseName || "";
      const parsedLocCode = parsed.locCode || "";
      const parsedLocName = parsed.locName || "";
...
const normalizeRetrievedLots = (lots = [], sourceRow = {}) =>
  ...
  return {
    ...
    convertedUomCode:
      getPOField(r, "convertedUomCode", "CONVERTED_UOM_CODE") || r.uomCode || "",
    convertedQuantity: formatNumber(
      getPOField(r, "convertedQuantity", "CONVERTED_QUANTITY") || r.rrQty || r.quantity || 0,
      decQty,
    ),
    conversionFactor:
      parseFormattedNumber(getPOField(r, "conversionFactor", "CONVERSION_FACTOR")) || 1,
...
  };
  ...
  const handleCloseFGLookup = async (selectedPayload) => {
    if (!selectedPayload) {
    ...
    const buildFGRRRow = async (selectedItem, baseRow = {}) => {
      ...
      const selectedVatCode = firstValue(
      ...
      const selectedVatName = firstValue(
      ...
      const selectedVatLookupRate = firstValue(
      ...
      const selectedUomCode = getSelectedUomCode(selectedItem);

      return recalcFGRRRowWithCurrencyRate(getHeaderWarehouseLocationFields({
        ...baseRow,
        invType: "FG",
        unitCost: formatNumber(parseFormattedNumber(selectedUnitCost), decUcost),
        vatCode: selectedVatCode || baseRow.vatCode || "",
        vatName: selectedVatName || baseRow.vatName || "",
        vatRate:
        ...
        categCode: selectedItem.categCode || baseRow.categCode || "",
        ...
        qtyAlloc: baseRow.qtyAlloc || "0.000000",
        qtyNeeded: baseRow.qtyNeeded || "0.000000",
        uomCode2: baseRow.uomCode2 || "",
        uomQty2: baseRow.uomQty2 || "0.000000",
        dateNeeded: baseRow.dateNeeded || today,
        itemSpecs: baseRow.itemSpecs || "",
        serviceCode: baseRow.serviceCode || "",
        serviceName: baseRow.serviceName || "",
        poQty: baseRow.poQty || "0.000000",
    ...
    if (selectedRowIndex !== null && selectedRowIndex !== undefined) {
      ...
      updatedRows[selectedRowIndex] = await buildFGRRRow(selectedItem, currentRow);
    ...
    const newRow = await buildFGRRRow(selectedItem);
...
  };
  ...
  const handleAddRowClick = () => {
    ...
    const normalizedTranType = String(state.fgrrTranType || "FGRR01")
      .trim()
      .toUpperCase();

    if (
      normalizedTranType === "FGRR01" &&
...
  };
  ...
  const handleConfirmLotPicking = () => {
    ...
    updatedRows[lotPickingRowIndex] = recalcFGRRRowWithCurrencyRate(
      {
        ...currentRow,
...
  };
  ...
  const handleConvertedUomSelect = async (selected) => {
    ...
    try {
      ...
      rows[index] = {
        ...row,
        convertedUomCode: selected.uomCode,
        conversionFactor,
        convertedQuantity: formatNumber(
          (parseFormattedNumber(row.rrQty) || 0) / conversionFactor,
          decQty,
        ),
      };
...
    // ✅ apply any extra lookup fields (names, etc.)
    ...
    if (isInventoryConversionEnabled && field === "convertedQuantity") {
      ...
      row.rrQty = formatNumber(
        (parseFormattedNumber(row.convertedQuantity) || 0) * conversionFactor,
        decQty,
      );
      row.lotDetails = [];
    }
    ...
    if (isInventoryConversionEnabled && field === "rrQty") {
      ...
      row.convertedQuantity = formatNumber(
        (parseFormattedNumber(row.rrQty) || 0) / conversionFactor,
        decQty,
      );
    }
...
    }
    ...
    if (["rrQty", "convertedQuantity"].includes(field) && !validateRRQtyWithinPOBalance(updatedRows)) {
      const originalRow = fgrrQuantityEditStartRef.current[index] || rows[index];
...
      }
      ...
      const getRowLotEntriesForSave = (row = {}) => {
        ...
        if (lotDetails.length > 0) {
          return lotDetails
            .map((lot) => ({
              ...lot,
              lotNo: String(lot.lotNo || lot.lot_no || "").trim(),
              quantity: parseFormattedNumber(lot.quantity || lot.rrQty || lot.rrQuantity || 0) || 0,
            }))
            .filter((lot) => lot.lotNo && lot.quantity > 0);
        }
        ...
        if (!fallbackLotNo || fallbackQty <= 0) return [];
        ...
        return [
          {
            ...
            rrQty: fallbackQty,
            rrQuantity: fallbackQty,
            bbDate: row.bbDate || null,
            qstatCode: row.qstatCode || row.qsCode || "",
            qsCode: row.qsCode || row.qstatCode || "",
            whouseCode: row.whouseCode || row.whCode || state.WHCode || state.WHcode || "",
            whCode: row.whCode || row.whouseCode || state.WHCode || state.WHcode || "",
      ...
      // DT1 must remain one row per item. Do not explode DT1 per lot.
      const dt1Payload = (state.detailRows || []).map((r, index) => {
        ...
        return {
          ...
          convertedQuantity: parseFormattedNumber(
            r.convertedQuantity ?? r.rrQty ?? r.quantity ?? 0,
          ),
      ...
      // DT3 is the official lot breakdown payload: one row per split lot quantity.
      const dt3Payload = [];
      ...
      (state.detailRows || []).forEach((r, rowIndex) => {
        ...
        sourceLots.forEach((lot, lotIndex) => {
          const lotQty = parseFormattedNumber(lot.quantity || lot.rrQty || lot.rrQuantity || 0);
          const lotNo = String(lot.lotNo || lot.lot_no || "").trim();
          ...
          dt3Payload.push({
            ...
            unitCost: parseFormattedNumber(lot.unitCost || lot.unit_cost || r.unitCost || r.unit_cost || 0),
...
          new Date().toISOString().split("T")[0],
        fgrrTranType: String(state.fgrrTranType || "FGRR01")
          .trim()
          .toUpperCase(),
      ...
      if (action === "Upsert" && shouldAutoGenerateGLOnSave) {
        ...
        if (!Array.isArray(glRowsForSave) || glRowsForSave.length === 0) {
          Swal.fire({
            ...
            text: "No GL entries were generated. Please check the FG Category account setup.",
          });
          return;
        }
...
          "";
        ...
        if (!isCloseAction && (!savedId || !savedNo)) {
          useSwalErrorAlert(
            "Invalid Save Response",
            "FGRR did not return the generated RR number and transaction ID.",
          );
          return;
        }
...
  };
  ...
  const handleConfirmCloseTransaction = async (confirmation) => {
    ...
    try {
      ...
      const response = await postRequest("closeFGRR", {
        ...
        text:
          validationCode === "INVALID_CREDENTIALS"
            ...
            : validationCode === "MISSING_CREDENTIALS"
              ...
              : error?.response?.data?.message || error?.message || "Unable to close transaction.",
      });
    } finally {
...
    }
    ...
    let nextCurrCode = currCode || state.currCode || glCurrDefault || "PHP";
    ...
    if (selectedCurrCode) {
      ...
      const rate =
        selectedCurrCode === normalizeCurrencyCode(glCurrDefault)
          ? defaultCurrRate
...
    if (code) {
      ...
      const result = await useTopCurrencyRow(selectedCode);
      if (result) {
        const rate =
          selectedCode === normalizeCurrencyCode(glCurrDefault)
            ? defaultCurrRate
            : await useTopForexRate(selectedCode, header.rr_date);
        const nextRate = formatNumber(parseFormattedNumber(rate), 6);
        const nextRows = (detailRows || []).map((row) =>
          recalcFGRRRowWithCurrencyRate({
            ...row,
            currCode: selectedCode,
            currRate: nextRate,
          }),
        );
...
        updateState({
          currCode: selectedCode,
          currName:
            normalizeCurrencyCode(getCurrencyCode(result) || selectedCode) === selectedCode
              ? getCurrencyName(result) || selectedCode
              : selectedCode,
          detailRows: nextRows,
...
  };
  ...
  const formatFGRRByField = (field, value) => {
    ...
    return formatNumber(value, decimalsByField[field] ?? 2);
  };

  const focusFGRRDetailCell = (field, nextIndex) => {
  ...
  const focusNextFGRRDetailCell = (index, field) => {
    if (typeof focusNextFGRRDetailRowInput === "function") {
      focusNextFGRRDetailRowInput(index, field, {
        rows: detailRowsRef.current || detailRows,
        zeroClearFields: fgrrDetailEnterNextRowZeroClearFields,
    ...
    if (nextIndex > index) {
      focusFGRRDetailCell(field, nextIndex);
    }
  };

  const handleFGRRGridKeyDown = async (e, index, field, options = {}) => {
  ...
  const fgrrNumericInput = (row, index, field, options = {}) => {
  ...
  const handleAddBlankRow = (index) => {
    ...
    const newRow = recalcFGRRRowWithCurrencyRate(getHeaderWarehouseLocationFields({
      invType: currentRow.invType || "FG",
      operation: "I",
  ...
  const renderFGRRDetailCell = (columnKey, row, index) => {
    const columnWidth = getFGRRDetailFallbackWidth(columnKey);
    const style = getFGRRDetailCellStyle(columnKey, columnWidth);
    ...
    const textInput = (field, options = {}) => (
      <input
        ...
        onKeyDown={(e) => handleFGRRGridKeyDown(e, index, field, options)}
    ...
    const detailColumnRenderers = {
      ...
      rrQty: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          {fgrrNumericInput(row, index, "rrQty")}
        </td>
      ),
      freeQty: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          {fgrrNumericInput(row, index, "freeQty")}
        </td>
      ),
      convertedUomCode: () => (
        <td key={columnKey} className="global-tran-td-ui relative" style={style}>
      ...
      convertedQuantity: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          {fgrrNumericInput(row, index, "convertedQuantity")}
        </td>
      ),
      unitCost: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
      ...
      bbDate: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          <input
            ...
            onKeyDown={(e) => handleFGRRGridKeyDown(e, index, "bbDate", { readOnly: isFormDisabled })}
          />
        </td>
      ),
      qstatCode: () => (
  ...
  const renderFGRRGlCell = (columnKey, row, index) => {
    const columnWidth = getFGRRGlFallbackWidth(columnKey);
    const style = getFGRRGlCellStyle(columnKey, columnWidth);
    const rowLocked = isFormDisabled || row.operation === "S";

    const focusNextGlCell = (field) => {
      if (typeof focusNextFGRRGlRowInput === "function") {
        focusNextFGRRGlRowInput(index, field, {
          rows: detailRowsGLRef.current || detailRowsGL,
          zeroClearFields: fgrrGlEnterNextRowZeroClearFields,
          parseValue: parseFormattedNumber,
    ...
    const amountInput = (field) => (
      <input
        ...
        onFocus={(e) => {
          ...
          if (typeof clearFGRRGlZeroOnFocus === "function") {
            clearFGRRGlZeroOnFocus(e, {
              isEditable: true,
...
                />
                ...
                <FieldRenderer
                  ...
                  required
                  type="select"
                  value={state.fgrrTranType || "FGRR01"}
                  disabled={isFormDisabled || detailRows.length > 0}
                  onChange={(val) => updateState({ fgrrTranType: val })}
                  options={(state.fgrrTranTypes || []).map((type) => ({
...
            <div className="global-tran-table-main-sub-div-ui">
              <table className="min-w-full border-separate border-spacing-0 [&_th]:border-b [&_th]:border-slate-200 [&_td]:border-t-0 [&_td]:border-l-0 [&_td]:border-r [&_td]:border-b [&_td]:border-slate-200 [&_tr>td:first-child]:border-l">
                <thead className="global-tran-thead-div-ui">
                  <tr>
                    {visibleFGRRDetailColumns.map((column) =>
                      renderFGRRDetailHeader(column.label, column.key, column.width, {
                        orderedColumns: visibleFGRRDetailColumns,
                      }),
                    )}
                    {!isFormDisabled && (
                      <th
                ...
                <tbody className="relative">
                  {sortedFGRRDetailRows.map(({ row, originalIndex }) => (
                    <tr key={originalIndex} className="global-tran-tr-ui">
                      {visibleFGRRDetailColumns.map((column) =>
                        renderFGRRDetailCell(column.key, row, originalIndex),
                      )}
                      {!isFormDisabled && (
                        <td
              ...
              {renderFGRRDetailHeaderContextMenu?.()}
...
                          {!isFormDisabled && !isRegularReceiving &&
                            (
                              <FontAwesomeIcon
                                icon={faMagnifyingGlass}
                                className="absolute top-1/2 right-2 -translate-y-1/2 text-blue-600 text-lg cursor-pointer hover:text-blue-900"
            ...
            <div className="global-tran-table-main-div-ui">
              <div className="global-tran-table-main-sub-div-ui">
                <table className="min-w-full border-separate border-spacing-0 [&_th]:border-b [&_th]:border-slate-200 [&_td]:border-t-0 [&_td]:border-l-0 [&_td]:border-r [&_td]:border-b [&_td]:border-slate-200 [&_tr>td:first-child]:border-l">
                  <thead className="global-tran-thead-div-ui">
                    <tr>
                      {orderedFGRRGlColumns.map((column) =>
                        renderFGRRGlHeader(column.label, column.key, column.width, {
                          orderedColumns: orderedFGRRGlColumns,
                        }),
                      )}
                    </tr>
                  </thead>
                  ...
                  <tbody className="relative">
                    {sortedFGRRGlRows.map(({ row, originalIndex }) => (
                      <tr
                        key={`${row.acctCode || row.id || "gl"}-${originalIndex}`}
                        className="global-tran-tr-ui"
                      >
                        {orderedFGRRGlColumns.map((column) =>
                          renderFGRRGlCell(column.key, row, originalIndex),
                        )}
                      </tr>
                    ))}
                  </tbody>
                </table>
                {renderFGRRGlHeaderContextMenu?.()}
              </div>
            </div>
...
      {/* MODALS */}
      {convertedUomLookup.isOpen && (
        <RequiredUomLookupModal
          isOpen={convertedUomLookup.isOpen}
          title="Select Converted UOM"
  ...
  <GlobalCombinedLookup
    ...
    detailColumns={
      openPORRColDetail.length > 0
        ? withoutCurrencyRateColumn(openPORRColDetail)
        : withoutCurrencyRateColumn(openPOColDetail)
    }
    ...
    fetchDetailApi={async (selectedIds) => {
      ...
      const currencyRateByPoNo = Object.fromEntries(
        selectedSummaries
          .map((row) => [
            getPOField(row, "PoNo", "PO_NO", "poNo", "po_no"),
            getPOField(row, "CurrRate", "CURR_RATE", "currRate", "curr_rate"),
          ])
          .filter(([poNo]) => poNo),
      );
      ...
      const normalizedRows = rows.map((row, index) => {
        ...
        const poCurrencyRate =
          normalized.currRate ||
          normalized.CurrRate ||
          currencyRateByPoNo[normalized.poNo || normalized.PoNo] ||
          1;
        const uniqueGroupId = [
        ...
        return {
          ...fillConfiguredOpenPODetailKeys(
            {
              ...normalized,
              CurrRate: poCurrencyRate,
              CURR_RATE: poCurrencyRate,
              currRate: poCurrencyRate,
              curr_rate: poCurrencyRate,
            },
            withoutCurrencyRateColumn(
...
          onGetSelectedItems={handleCloseFGLookup}
          onCancel={() =>
            updateState({ msLookupModalOpen: false, selectedRowIndex: null })
          }
