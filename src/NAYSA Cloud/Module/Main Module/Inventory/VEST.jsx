...
const getVESTSaveResult = (response) => {
...
const PostVEST = ({ isOpen, onClose, userCode, docType = "VEST", documentTitle = "VE Stock Transfer", detailsRoute = "/page/VEST", fieldNo = "vestNo" }) => {
  ...
  const pickDocAndBranch = (row) => ({
    docNo: row?.[fieldNo] || row?.vestNo || row?.docNo || null,
...
const VEST = () => {
  ...
  const decQty = companyInfo?.itemDecqtyVE ?? 2;
  const decUcost = companyInfo?.itemDecUcostVE ?? 6;
  ...
  const docType = docTypes.VEST || "VEST";
  const hsDoc = getAllTopHSDocRow?.(docType);
  const pdfLink = docTypePDFGuide[docType];
  const videoLink = docTypeVideoGuide[docType];
  const documentTitle = hsDoc?.docName || docTypeNames[docType] || "VE Stock Transfer";
  const detailsRoute = "/page/VEST";
  const documentNoField = "vestNo";
  ...
  const getDefaultTranType = (list = tranTypes) => {
    ...
    return (
      getTranTypeCode(
        rows.find((x) => normalizeCode(getTranTypeCode(x)) === "VEST04"),
      ) ||
      getTranTypeCode(
  ...
  const loadTranTypes = async ({ allowDbFallback = true } = {}) => {
    ...
    const [dbRows1, dbRows2] = await Promise.all([
      useTopDocDropDown(docType, "TRAN_TYPE").catch((error) => {
        console.warn("Unable to load VEST TRAN_TYPE using docType/type order.", error);
        return [];
      }),
      useTopDocDropDown("TRAN_TYPE", docType).catch((error) => {
        console.warn("Unable to load VEST TRAN_TYPE using type/docType order.", error);
        return [];
      }),
    ]);
  ...
  const loadCompanyData = async () => {
    ...
    try {
      const tbls = "vest_hd,vest_dt1,vest_dt2";
  ...
  const fetchTranData = async (documentNo, branchCode, direction = "") => {
    ...
    try {
      ...
      const retrievedDetailRows = (data.dt1 || []).map((item) => ({
        ...
        engineNo: item.engineNo || item.engine_no || "",
        prodNo: item.prodNo || item.prod_no || "",
        color: item.color || "",
        chassisNo: item.chassisNo || item.csNo || item.cs_no || "",
        pnpNo: item.pnpNo || item.pnp_no || "",
        csrNo: item.csrNo || item.csr_no || "",
      ...
      updateState({
        documentStatus: data.vestStatus,
        status: data.docStatus,
        noReprints: data.noReprints,
        documentID: data.vestId,
        documentNo: data.vestNo,
        branchCode: data.branchCode,
        documentDate: useformatToDatev2(data.vestDate),
  ...
  const handleActivityOption = async (action) => {
    ...
    const getFormattedPayload = (targetGLRows) => {
      ...
      return {
        ...
        vestNo: documentNo || "",
        vestId: documentID || "",
        vestDate: toDateInputValue(documentDate),
  ...
  const handleAddRow = async () => {
    ...
    await handleOpenVELookup(false);
    return;
  };
  ...
  const handleCancel = async () => {
    ...
    if (!detailRows || detailRows.length === 0) {
      useSwalInfoAlert("Cancel Transaction", "No vehicle details found to cancel.");
      return;
    }
  ...
  const vehicleFieldMaxLengths = {
    make: useGetFieldLength(tblFieldArray, "make"),
    model: useGetFieldLength(tblFieldArray, "model"),
    serialNo: useGetFieldLength(tblFieldArray, "serial_no"),
    engineNo: useGetFieldLength(tblFieldArray, "engine_no"),
    prodNo: useGetFieldLength(tblFieldArray, "prod_no"),
    color: useGetFieldLength(tblFieldArray, "color"),
    chassisNo: useGetFieldLength(tblFieldArray, "cs_no"),
    pnpNo: useGetFieldLength(tblFieldArray, "pnp_no"),
    csrNo: useGetFieldLength(tblFieldArray, "csr_no"),
  };

  const vestDetailColumnDefs = [
  ...
  const {
    getSortedRows: getSortedVestDetailRows,
    clearAllSorting: clearVestDetailSorting,
    renderHeaderContextMenu: renderVestDetailHeaderContextMenu,
    renderResizableHeader: renderVestDetailHeader,
  } = useResizableTableColumns(visibleVestDetailColumns);
  const sortedVestDetailRows = getSortedVestDetailRows(
    detailRows.map((row, originalIndex) => ({ row, originalIndex })),
    (entry, sortKey) => sortKey === "ln" ? entry.originalIndex + 1 : entry.row?.[sortKey] ?? "",
  );

  const vestGlColumnDefs = [
    { key: "ln", label: "LN", width: 56 },
  ...
  const {
    getSortedRows: getSortedVestGlRows,
    clearAllSorting: clearVestGlSorting,
    renderHeaderContextMenu: renderVestGlHeaderContextMenu,
    renderResizableHeader: renderVestGlHeader,
  } = useResizableTableColumns(vestGlColumnDefs);
  const sortedVestGlRows = getSortedVestGlRows(
  ...
  const handleDetailChange = async (
    ...
    if (field === "itemCode") {
      ...
      row["make"] = value.make ?? value.carMake ?? value.car_make ?? "";
      row["modelYear"] = value.modelYear ?? value.modelYr ?? value.model_yr ?? "";
      row["model"] = value.model ?? "";
      row["serialNo"] = value.serialNo ?? value.serial_no ?? "";
      row["engineNo"] = value.engineNo ?? value.engine_no ?? "";
      row["prodNo"] = value.prodNo ?? value.prod_no ?? "";
      row["color"] = value.color ?? "";
      row["chassisNo"] = value.chassisNo ?? value.csNo ?? value.cs_no ?? "";
      row["pnpNo"] = value.pnpNo ?? value.pnp_no ?? "";
      row["csrNo"] = value.csrNo ?? value.csr_no ?? "";

      // Auto-populate the inventory account tagged in the VE category data.
  ...
  const handleOpenVELookup = async (itemSingleSelect) => {
    try {
      ...
      const parseLookupRows = (response) => {
        const rawResult =
        ...
        if (typeof rawResult !== "string" || !rawResult.trim()) return [];
      ...
      try {
        response = await fetchDataJson(endpoint, lookupPayload);
        vehicleRows = parseLookupRows(response);
      } catch (primaryError) {
        console.warn(
          "VEST-specific vehicle lookup was unavailable; trying VE balance mode.",
          primaryError,
        );
      }

      // Some VE lookup procedures only expose existing location balances
      // through the VEAJ inventory-loss lookup mode. VEST still saves and
      // processes as VEST; this fallback is only for selecting source vehicles.
      ...
      if (vehicleRows.length === 0) {
        useSwalInfoAlert("Vehicle Location Balance", "No records found");
        return;
      }

      updateState({
        globalLookupRow: vehicleRows,
        globalLookupHeader: Array.isArray(colConfig) ? colConfig : [],
        veLookupModalOpen: true,
      });
    } catch (error) {
      console.error("VEST vehicle lookup error:", error);
      useSwalErrorAlert(
        "Vehicle Location Balance",
        error?.message || "Unable to load vehicle records.",
      );
      updateState({
        globalLookupRow: [],
        globalLookupHeader: [],
  ...
  const handleCloseVELookup = (selectedItems) => {
    ...
    const validItems = itemsArray.filter(
      (item) => item && typeof item === "object" && item.itemCode,
    );
    if (validItems.length === 0) {
      useSwalInfoAlert("Vehicle Selection", "No valid vehicle was selected.");
      return;
    }

    const newRows = validItems.flatMap((item) => {
      const rawQtyHand = parseFormattedNumber(
      ...
      if (itemSingleSelect && selectedTranType === "IR") {
        ...
        updateState({ itemSingleSelect: false, veLookupModalOpen: false });
        return [];
      }
  ...
  return (
    <div className="global-tran-main-div-ui">
        ...
        <div className="global-tran-header-div-ui">
          ...
          >
            <div className="col-span-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {/* Column 1: Branch, VEST No., VEST Date */}
                  ...
                  <FieldRenderer
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        handleDocNoBlur();
                        e.preventDefault();
                        document.getElementById("vestDate")?.focus();
                      }
                    }}
                    placeholder=" "
                ...
                <div className="relative w-full">
                  ...
                  >
                    <DateFormatInput
                      ...
                      updateState={(updates) => {
                        if (updates.vestDate !== undefined) {
                          updateState({ documentDate: updates.vestDate });
                  ...
                  <label htmlFor="vestDate" className="global-ref-floating-label">
                    VEST Date
                  </label>
                </div>
              </div>
            ...
            <div className="global-tran-table-main-sub-div-ui">
              <table className="min-w-full border-separate border-spacing-0 [&_th]:border-b [&_th]:border-slate-200 [&_td]:border-t-0 [&_td]:border-l-0 [&_td]:border-r [&_td]:border-b [&_td]:border-slate-200 [&_tr>td:first-child]:border-l">
                <thead className="global-tran-thead-div-ui">
                  <tr>
                    {visibleVestDetailColumns.map((column) => (
                      <Fragment key={`detail-header-${column.key}`}>
                        {renderVestDetailHeader(column.label, column.key, column.width, {
                          orderedColumns: visibleVestDetailColumns,
                        })}
                      </Fragment>
                    ))}
                    {!isFormDisabled && (
                  ...
                  {renderVestDetailHeaderContextMenu()}
                </thead>
                  ...
                  {sortedVestDetailRows.map(({ row, originalIndex: index }) => (
                    <tr key={`${row.uniqueKey || row.itemCode || "row"}-${index}`} className="global-tran-tr-ui">
                      ...
                      {[
                        ...
                        <td key={`vehicle-${field}`} className="global-tran-td-ui">
                          <input
                            type="text"
          ...
          <div className="global-tran-table-main-div-ui">
            <div className="global-tran-table-main-sub-div-ui">
              <table className="min-w-full border-separate border-spacing-0 [&_th]:border-b [&_th]:border-slate-200 [&_td]:border-t-0 [&_td]:border-l-0 [&_td]:border-r [&_td]:border-b [&_td]:border-slate-200 [&_tr>td:first-child]:border-l">
                <thead className="global-tran-thead-div-ui">
                  <tr>
                    {vestGlColumnDefs.map((column) => (
                      <Fragment key={`gl-header-${column.key}`}>
                        {renderVestGlHeader(column.label, column.key, column.width, {
                          orderedColumns: vestGlColumnDefs,
                        })}
                      </Fragment>
                    ))}
                  </tr>
                ...
                <tbody className="relative">
                  {sortedVestGlRows.map(({ row, originalIndex: index }) => (
      ...
      <div className={topTab === "history" ? "" : "hidden"}>
        <AllTranHistory
          showHeader={false}
          endpoint="/getVESTHistory"
          cacheKey={`VEST:${state.branchCode || ""}:${state.documentNo || ""}`}
          activeTabKey="VEST_Summary"
