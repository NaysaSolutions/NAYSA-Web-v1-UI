...
const PostFGRTV = ({ isOpen, onClose, userCode }) => {
      ...
      try {
        ...
        if (postingRows.length === 0 && !alertFired.current) {
          useSwalValidationAlert({
            ...
            message: "There are no FG Return to Vendor records to post.",
          });
        ...
        console.error("Error fetching FGRTV posting data:", error);

        useSwalValidationAlert({
          icon: "error",
          title: "Posting Load Error",
          message: "Unable to load FG Return to Vendor records for posting.",
        });
  ...
  const pickDocAndBranch = (row) => {
    ...
    const docNo = row.fgrtvNo || row.rtvNo || row.docNo || row.FGRTVNo || row.FGRTV_NO || null;
  ...
  const handleViewDocument = (row) => {
    ...
    if (!docNo || !branchCode) {
      useSwalValidationAlert({
        icon: "warning",
        title: "Missing Keys",
        message: "Cannot determine FGRTV No. or Branch Code.",
      });
      return;
    }

    const FGRTV_VIEW_URL = "/page/FGRTV";
    const url =
      `${window.location.origin}${FGRTV_VIEW_URL}` +
      `?fgrtvNo=${encodeURIComponent(docNo)}` +
