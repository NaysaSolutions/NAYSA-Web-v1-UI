...
import PostVERTV from "./PostVERTV.jsx";
  ...
  const handleReset = () => {
   ...
   const loadCompanyData = async () => {
    ...
    try {
     ...
     const tbls = 'vertv_hd,vertv_dt1,vertv_dt2'
...
const fetchTranData = async (documentNo, branchCode,direction='') => {
  ...
  try {
    const data = await useFetchTranData(documentNo, branchCode,docType,"vertvNo",direction);
    ...
    const formattedGLRows = (data.dt2 || []).map(glRow => ({
      ...
      slRefDate: toDateInputValue(data.vertvDate),
      slrefDate: toDateInputValue(data.vertvDate),
    }));
    ...
    updateState({
      ...
      documentID: data.vertvId,
      documentNo: data.vertvNo,
      branchCode: data.branchCode,
      documentDate: useformatToDatev2(data.vertvDate),
      vendCode: data.vendCode,
      vendName: data.vendName,
      poNo: data.poNo || "",
      rrNo: data.rrNo || "",
      rtvrrAmt: formatNumber(data.rtvrrAmt || 0, 2),
...
const handleActivityOption = async (action) => {
  ...
  const getFormattedPayload = (targetGLRows) => {
    ...
    return {
      ...
      vertvNo: documentNo || "",
      vertvId: documentID || "",
      vertvDate: documentDate,
      vendCode: vendCode || "",
      vendName: vendName || "",
      poNo: poNo || "",
      rrNo: rrNo || "",
      rtvrrAmt: parseFormattedNumber(rtvrrAmt || 0),
      whCode: headerWhCode,
      locCode: locCode || "",
      refDocNo1: refDocNo1 || "",
      refDocNo2: refDocNo2 || "",
      ...
      dt1: detailRows.map((row, index) => ({
        ...
        veId: row.veId || "",
        make: row.make || "",
        modelYr: row.modelYr || row.modelYear || "",
        model: row.model || "",
        serialNo: row.serialNo || "",
        engineNo: row.engineNo || "",
        prodNo: row.prodNo || "",
        color: row.color || "",
        csNo: row.csNo || row.chassisNo || "",
        pnpNo: row.pnpNo || "",
        csrNo: row.csrNo || "",
        lotNo: row.lotNo || "",
  ...
  try {
    ...
    if (action === "Upsert") {
      ...
      const response = await useTransactionUpsert(docType, savePayload, updateState, 'vertvId', 'vertvNo');

      if (response) {
        const isZero = Number(noReprints) === 0;
        const onSaveAndPrint = isZero
          ? () => updateState({ showSignatoryModal: true })
          : () => handleSaveAndPrint(response.data[0].vertvId);
...
const handleGetItem = async (index = null) => {
  ...
  const newRow = {
    ...
    slCode: "",
    uniqueKey: "",
    make: "", modelYr: "", model: "", serialNo: "", engineNo: "", prodNo: "", color: "", csNo: "", pnpNo: "", csrNo: ""
  };
...
const handleColumnLabel = (columnName) =>{
  ...
  const vertvDetailColumnDefs = useMemo(() => [
    { key: "ln", label: "LN", width: 56 },
    { key: "veId", label: "Vehicle ID", width: 130 },
    { key: "itemCode", label: "Item Code", width: 120 },
    { key: "itemName", label: "Item Name", width: 260 },
    { key: "make", label: "Make", width: 120 },
    { key: "modelYr", label: "Model Year", width: 110 },
    { key: "model", label: "Model", width: 120 },
    { key: "serialNo", label: "Serial No", width: 130 },
    { key: "engineNo", label: "Engine No", width: 130 },
    { key: "prodNo", label: "Product No", width: 130 },
    { key: "color", label: "Color", width: 110 },
    { key: "csNo", label: "Chassis No", width: 130 },
    { key: "pnpNo", label: "PNP No", width: 120 },
    { key: "csrNo", label: "CSR No", width: 120 },
  ...
  } = useResizableTableColumns(vertvDetailColumnDefs);

  const orderedFgrtvDetailColumns = getOrderedFgrtvDetailColumns(vertvDetailColumnDefs);
  ...
  const getFgrtvDetailFallbackWidth = (key) => vertvDetailColumnDefs.find((column) => column.key === key)?.width || 120;
  ...
  const vertvGlColumnDefs = useMemo(() => [
    { key: "ln", label: "LN", width: 56 },
  ...
  } = useResizableTableColumns(vertvGlColumnDefs);

  const orderedFgrtvGlColumns = getOrderedFgrtvGlColumns(vertvGlColumnDefs);
  const getFgrtvGlFallbackWidth = (key) => vertvGlColumnDefs.find((column) => column.key === key)?.width || 120;
  ...
  const vertvDetailEnterNextRowZeroClearFields = ["quantity", "unitCost"];
  const vertvGlEnterNextRowZeroClearFields = ["debit", "credit", "debitFx1", "creditFx1", "debitFx2", "creditFx2"];
  ...
  const handleDownloadSingleUploadTemplate = async () => {
    ...
    await downloadGlobalSingleUploadTemplate({
      ...
      fileName: "VE RTV Single Transaction Uploading Template.xlsx",
...
useEffect(() => {
  ...
  const docNo = params.get("vertvNo");
...
const handleCloseBranchModal = (selectedBranch) => {
  ...
  const handleOpenVELookup = async () => {
    try {
      updateState({ isLoading: true });
  
      const response = await fetchDataJson("getInvLookupVE", { userCode, branchCode, whouseCode: state.WHcode || state.whouseCode || "", locCode: state.locCode || "", docType: "VERTV", tranType: "IR" });
      const custData = response?.data?.[0]?.result ? JSON.parse(response.data[0].result) : [];
  

      const colConfig = await useSelectedHSColConfig("getInvLookupVE", userCode);


     if (custData.length === 0) {
        useSwalErrorAlert("Vehicle Location Balance","No records found")
         updateState({ isLoading: false });
        return; 
      }
      ...
      console.error("Failed to fetch vehicle location balance:", error);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to fetch vehicle location balance.",
      });
      updateState({ 
          globalLookupRow: [] ,
          globalLookupHeader: [] });
  ...
  const handleCloseVELookup = (selectedItems) => {
  updateState({ veLookupModalOpen: false });
  ...
  const newRows = itemsArray.map((item) => ({
    ...
    veId: item?.veId ?? "",
    make: item?.make ?? "",
    modelYr: item?.modelYr ?? item?.modelYear ?? "",
    model: item?.model ?? "",
    serialNo: item?.serialNo ?? "",
    engineNo: item?.engineNo ?? "",
    prodNo: item?.prodNo ?? "",
    color: item?.color ?? "",
    csNo: item?.csNo ?? item?.chassisNo ?? "",
    pnpNo: item?.pnpNo ?? "",
    csrNo: item?.csrNo ?? "",
...
const renderFgrtvDetailColumn = (columnKey, row, index) => {
  ...
  const focusNextDetailCell = (field) => {
    focusNextFgrtvDetailRowInput(index, field, {
      rows: detailRows,
      zeroClearFields: vertvDetailEnterNextRowZeroClearFields,
  ...
  const detailColumnRenderers = {
    ...
    veId: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("veId", { readOnly: true })}</td>,
    ...
    make: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("make", { readOnly: true })}</td>,
    modelYr: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("modelYr", { readOnly: true })}</td>,
    model: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("model", { readOnly: true })}</td>,
    serialNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("serialNo", { readOnly: true })}</td>,
    engineNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("engineNo", { readOnly: true })}</td>,
    prodNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("prodNo", { readOnly: true })}</td>,
    color: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("color", { readOnly: true })}</td>,
    csNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("csNo", { readOnly: true })}</td>,
    pnpNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("pnpNo", { readOnly: true })}</td>,
    csrNo: () => <td key={columnKey} className="global-tran-td-ui" style={style}>{textInput("csrNo", { readOnly: true })}</td>,
...
const renderFgrtvGlColumn = (columnKey, row, index) => {
  ...
  const focusNextGlCell = (field) => focusNextFgrtvGlRowInput(index, field, { rows: detailRowsGL, zeroClearFields: vertvGlEnterNextRowZeroClearFields, parseValue: parseFormattedNumber, onClearNextValue: (nextIndex, nextField, value) => handleDetailChangeGL(nextIndex, nextField, value) });
  const modalHandlers = {
  ...
  <div className="global-tran-header-ui">
            ...
            <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="global-tran-textbox-group-div-ui">
                      ...
                      <FieldRenderer
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            handleDocNoBlur();
                            e.preventDefault();
                            document.getElementById("vertvDate")?.focus();
                          }
                        }}
                        placeholder=" "
                    ...
                    <div className="relative w-full">
                      ...
                      <label htmlFor="vertvDate" className="global-ref-floating-label">VERTV Date</label>
                    </div>
                ...
                <FieldRenderer id="poNo" label="PO No." type="text" value={poNo || ""} onChange={(val) => updateState({ poNo: val })} disabled={isFormDisabled} placeholder=" " maxLength={useGetFieldLength(tblFieldArray, "po_no")} />
                <FieldRenderer id="rrNo" label="RR No." type="text" value={rrNo || ""} onChange={(val) => updateState({ rrNo: val })} disabled={isFormDisabled} placeholder=" " maxLength={useGetFieldLength(tblFieldArray, "rr_no")} />
    ...
    {showAllTranDocNo && (
      <AllTranDocNo
        ...
        params={{branchCode,branchName,docType,documentTitle,fieldNo : "vertvNo"}}
    ...
    <div className={topTab === "history" ? "" : "hidden"}>
      <AllTranHistory
        showHeader={false}
        endpoint="/getVERTVHistory"
        cacheKey={`VERTV:${state.branchCode || ""}:${state.docNo || ""}`}  // ✅ per-transaction
        activeTabKey="VERTV_Summary"
