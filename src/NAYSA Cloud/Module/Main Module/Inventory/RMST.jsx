...
if (field === "itemCode") {
  ...
  const handleCloseRMLookup = (selectedItems) => {
    ...
    const newRows = itemsArray.flatMap((item) => {
      ...
      // Receiver rows may fall back to RM_INT.CONTROL_NO.
      ...
      const selectedGroupId = firstValue(
        item?.groupId,
        item?.group_id,
        item?.GROUP_ID,
        "",
      ...
      const selectedControlNo = firstValue(
        item?.controlNo,
        item?.CONTROL_NO,
        "",
      );
      ...
      // RMST currently lets the user select the physical receiving location
      // on the detail row. Only the virtual destination is auto-filled.
      const destinationLocCode = senderFlow ? "INTRANSIT" : "";
