...
const [isViewDocument, setIsViewDocument] = useState(false);
  ...
  useEffect(() => {
    const p = new URLSearchPararm(location.search);
...
    activeTab: "basic",
    GLactiveTab: "invoice", // same pattern as rmAJ (optional)
...
  // PR.jsx
  const docType = docTypes?.FGRFP || "FGRFP";
...
  const statusColor = statusMap[displayStatus] || "";
  ...
  const FGRFPDetailColumnDefs = useMemo(
  () => [
  ...
  const {
    getColumnStyle: getFGRFPDetailColumnStyle,
    getFrozenColumnStyle: getFGRFPDetailFrozenStyle,
    getOrderedColumns: getOrderedFGRFPDetailColumns,
    getSortedRows: getSortedFGRFPDetailRows,
    clearZeroValueOnFocus: clearFGRFPDetailZeroOnFocus,
    focusNextRowInput: focusNextFGRFPDetailRowInput,
    renderHeaderContextMenu: renderFGRFPDetailHeaderContextMenu,
    renderResizableHeader: renderFGRFPDetailHeader,
  } = useResizableTableColumns(FGRFPDetailColumnDefs);

  const visibleFGRFPDetailColumns = useMemo(
  () => getOrderedFGRFPDetailColumns(FGRFPDetailColumnDefs),
  [getOrderedFGRFPDetailColumns, FGRFPDetailColumnDefs]
);

  const getFGRFPDetailFallbackWidth = (key) =>
    FGRFPDetailColumnDefs.find((column) => column.key === key)?.width || 120;
  const getFGRFPDetailCellStyle = (key, fallbackWidth) => ({
    ...getFGRFPDetailColumnStyle(key, fallbackWidth),
    ...getFGRFPDetailFrozenStyle(key, visibleFGRFPDetailColumns, fallbackWidth, {
      isHeader: false,
    }),
  });
 const sortedFGRFPDetailRows = useMemo(
  () =>
    getSortedFGRFPDetailRows(
  ...
  [getSortedFGRFPDetailRows, detailRows]
)

  const FGRFPDetailEnterNextRowZeroClearFields = ["rrQty", "freeQty", "unitCost"];

  const FGRFPGlColumnDefs = useMemo(
    () => [
      { key: "ln", label: "LN", width: 56 },
      { key: "acctCode", label: "Account Code", width: 120 },
      { key: "rcCode", label: "RC Code", width: 120 },
  ...
  const {
    getColumnStyle: getFGRFPGlColumnStyle,
    getFrozenColumnStyle: getFGRFPGlFrozenStyle,
    getOrderedColumns: getOrderedFGRFPGlColumns,
    getSortedRows: getSortedFGRFPGlRows,
    clearZeroValueOnFocus: clearFGRFPGlZeroOnFocus,
    focusNextRowInput: focusNextFGRFPGlRowInput,
    renderHeaderContextMenu: renderFGRFPGlHeaderContextMenu,
    renderResizableHeader: renderFGRFPGlHeader,
  } = useResizableTableColumns(FGRFPGlColumnDefs);

  const orderedFGRFPGlColumns = useMemo(
    () => getOrderedFGRFPGlColumns(FGRFPGlColumnDefs),
    [getOrderedFGRFPGlColumns, FGRFPGlColumnDefs],
  );

  const getFGRFPGlFallbackWidth = useCallback(
    (key) => FGRFPGlColumnDefs.find((column) => column.key === key)?.width || 120,
    [FGRFPGlColumnDefs],
  );

  const getFGRFPGlCellStyle = useCallback(
    (key, fallbackWidth) => ({
      ...getFGRFPGlColumnStyle(key, fallbackWidth),
      ...getFGRFPGlFrozenStyle(key, orderedFGRFPGlColumns, fallbackWidth, {
        isHeader: false,
      }),
    }),
    [getFGRFPGlColumnStyle, getFGRFPGlFrozenStyle, orderedFGRFPGlColumns],
  );

  const sortedFGRFPGlRows = useMemo(
    () =>
      getSortedFGRFPGlRows(
        (detailRowsGL || []).map((row, originalIndex) => ({ row, originalIndex })),
        (entry, sortKey) =>
          sortKey === "ln" ? entry.originalIndex + 1 : entry.row?.[sortKey] ?? "",
      ),
    [getSortedFGRFPGlRows, detailRowsGL],
  );

  const FGRFPGlEnterNextRowZeroClearFields = [
    "debit",
    "credit",
    "debitFx1",
    "creditFx1",
  ...
  const generateClientGroupId = () => {
    ...
    return `FGRFP-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`.toUpperCase();
  };
...
    const current = detailRows?.[rowIndex]?.itermpecs ?? "";
    updateState({
      specsModalOpen: true,
...
//     console.log("FGRFP Reference PO warehouse fetch", {
//       selection,
//       summary,
...
      // -------------------------------------------------------
      // 4) FIELD LENGTH CHECK (rmAJ style)
      // -------------------------------------------------------
      const lens = await useFieldLenghtCheck("FGRFP_hd,FGRFP_dt1,FGRFP_dt2");
...
  // ==========================
  ...
  const fetchTranData = async (rfpNo, branchCode, direction = "") => {
    ...
    try {
      ...
      const parsed = data;
      const parsedDocumentNo = parsed.rfpNo || parsed.FGRFPNo || parsed.documentNo || parsed.docNo || parsed.rrNo || "";
...
const parsedLocName =
  ...
  "";
      ...
      console.log("✅ FETCHED FGRFP DT3:", dt3);
      console.table(dt3);
...
const dt3ByItem = dt3.reduce((acc, lot) => {
  ...
  return acc;
      ...
      const normalizeRetrievedLots = (lots = [], sourceRow = {}) =>
        lots
          .map((lot, lotIndex) => ({
            ...
            // Background fields for FGRFP_DT3. These are kept in state/payload only and are not displayed in the Lot No Breakdown modal.
      ...
      const mappedDT1 = dt1.map((r, idx) => {
        ...
        // Retrieve Lot No Breakdown from FGRFP_DT3 by group_id only.
...
  };
  ...
  const pickFGRFPFirstValue = (...values) => {
  ...
  const getFGRFPItemCategoryCode = (item = {}) =>
    pickFGRFPFirstValue(
      item?.categCode,
      item?.categ_code,
      item?.CATEG_CODE,
  ...
  const getDefaultFGRFPAccountFromItem = (item = {}) => ({
  code: pickFGRFPFirstValue(
    item?.invAcctCode,
    item?.INVACCT_CODE,
    item?.invacctCode,
    item?.invacct_code,
  ...
  const resolveDefaultFGRFPAccount = async (item = {}) => {
    // Use the selected item lookup result only. Do not call getrmCategoryAccount.
    return getDefaultFGRFPAccountFromItem(item);
  };
  ...
  const handleClosermLookup = async (selectedPayload) => {
    ...
    const selectedIterm = Array.isArray(selectedPayload?.records)
      ? selectedPayload.records
      : Array.isArray(selectedPayload)
        ? selectedPayload
        : [selectedPayload];

    if (!selectedIterm.length) {
      updateState({ rmLookupModalOpen: false, selectedRowIndex: null });
      return;
    }
    ...
    const buildFGRFPRow = async (selectedItem) => {
      ...
      const defaultFGRFPAccount = await resolveDefaultFGRFPAccount(selectedItem);

      return recalcFGRFPRow(getHeaderWarehouseLocationFields({
        invType: "rm",
        ...
        categCode: getFGRFPItemCategoryCode(selectedItem),
        acctCode: defaultFGRFPAccount.code || "",
        acctName: defaultFGRFPAccount.name || "",
        poStatus: status || "",
    ...
    if (selectedIterm.length > 1 && selectedRowIndex === null) {
      const newRows = await Promise.all(selectedIterm.map(buildFGRFPRow));
    ...
    const selectedItem = selectedIterm[0];
    const selectedUnitCost = firstValue(
      selectedItem.unitCost,
      selectedItem.UnitCost,
      selectedItem.UNIT_COST,
    ...
    if (selectedRowIndex !== null && selectedRowIndex !== undefined) {
      ...
      updatedRows[selectedRowIndex] = recalcFGRFPRow(getHeaderWarehouseLocationFields({
        ...currentRow,
        groupId: ensureClientGroupId(currentRow),
        categCode: getFGRFPItemCategoryCode(selectedItem) || currentRow.categCode || "",
        acctCode: defaultFGRFPAccount.code || currentRow.acctCode || "",
...
    };
    ...
    const updatedRows = [...detailRows, recalcFGRFPRow(getHeaderWarehouseLocationFields(newRow))];
    updateState({
      detailRows: updatedRows,
      rmLookupModalOpen: false,
    });
...
          text: "Do you want to apply this Warehouse to all detail iterm?",
          icon: "info",
...
          Swal.fire({
            title: "Apply to Details?",
            text: "Do you want to apply this Location to all detail iterm?",
            icon: "info",
            showCancelButton: true,
            confirmButtonText: "Yes, update all",
            cancelButtonText: "No, header only",
...
  };
  ...
  const makeLotBreakdownRow = (row = {}, quantity = "") => ({
    ...
    // Background fields for FGRFP_DT3. Do not render these columns in the modal table.
  ...
  const handleConfirmLotPicking = () => {
    ...
    const validLots = lotEntryRows
      .map((lot, index) => ({
        ...
        // Background fields for FGRFP_DT3. Hidden in modal display.
    ...
    updatedRows[lotPickingRowIndex] = recalcFGRFPRow(
      {
        ...currentRow,
...
  };
  ...
  const sanitizeFGRFPNumeric = (value) => {
  ...
  const formatFGRFPByField = (field, value) => {
...
    if (Object.prototype.hasOwnProperty.call(numericFieldDecimals, field)) {
      const sanitized = sanitizeFGRFPNumeric(normalizedValue);
...
  const handleActivityOption = async (action) => {
  ...
  try {
    const isNew = !state.documentID;
      ...
      // DT1 must remain one row per FGRFP item, even when lot quantities are split.
...
      // Build payload (match your sproc pararm)
      const glData = {
...
      console.log("FGRFP upsert response:", docType, glData, updateState);
      // ================
      // UPSERT / SAVE
      // ================
      if (action === "Upsert") {
...
        console.log("FGRFP upsert response:", res);
        
        ...
        const normalizeSaveRow = (value) => {
          ...
          value.FGRFPNo = getReturnedValue(
            value,
            "rfpNo",
            "RFP_NO",
            "FGRFPNo",
          ...
          value.FGRFPHdId = getReturnedValue(
            value,
            "rfpHdId",
            "rfpId",
            "rfp_id",
...
            title: "Validation",
            html: String(
              row.errorrmg || "Please complete required fields.",
            ).replace(/\r?\n/g, "<br/>"),
          });
          return;
        }

        // accept common key variants returned by FGRFP upsert
        const savedId =
          row?.rfpHdId ||
          row?.rfpId ||
          row?.rfp_id ||
...
        // reflect auto-generated FGRFP No / FGRFP ID in UI
        updateState({
          documentID: savedId,
          documentNo: savedNo,
          isDocNoDisabled: true,
...
      }
    } catch (err) {
      console.error("FGRFP action error:", err);
      Swal.fire({
        icon: "error",
        title: "Error",
...
    // ✅ Ask backend to fill acctName/slName/rcName/vatName/atcName etc (same as rmAJ)
...
    // ✅ recompute Fx fields like rmAJ (editEntries API)
...
  useEffect(() => {
    const pararm = new URLSearchPararm(location.search);
    const docNo = pararm.get("poNo");
    const brCode = pararm.get("branchCode");
...
  };
  ...
  const focusFGRFPDetailCell = (field, nextIndex) => {
  ...
  const focusNextFGRFPDetailCell = (index, field) => {
    if (typeof focusNextFGRFPDetailRowInput === "function") {
      focusNextFGRFPDetailRowInput(index, field, {
        rows: detailRowsRef.current || detailRows,
        zeroClearFields: FGRFPDetailEnterNextRowZeroClearFields,
    ...
    if (nextIndex > index) {
      focusFGRFPDetailCell(field, nextIndex);
    }
  };

  const handleFGRFPGridKeyDown = (e, index, field, options = {}) => {
    ...
    if (e.key === "Enter") {
      ...
      if (["rrQty", "freeQty", "unitCost", "vatRate"].includes(field)) {
        ...
        e.target.value = formatFGRFPByField(
          field,
          Number.isFinite(numericValue) ? numericValue : 0,
        );
      }
      handleDetailChange(index, field, e.target.value, true);
      setTimeout(() => focusNextFGRFPDetailCell(index, field), 0);
      return;
    }

    if (e.key === "ArrowUp") {
      e.preventDefault();
      focusFGRFPDetailCell(field, Math.max(0, index - 1));
      return;
    }

    if (e.key === "ArrowDown") {
      e.preventDefault();
      focusFGRFPDetailCell(
        field,
  ...
  const renderFGRFPDetailCell = (columnKey, row, index) => {
    const columnWidth = getFGRFPDetailFallbackWidth(columnKey);
    const style = getFGRFPDetailCellStyle(columnKey, columnWidth);
    ...
    const textInput = (field, options = {}) => (
      <input
        ...
        onKeyDown={(e) => handleFGRFPGridKeyDown(e, index, field, options)}
    ...
    const detailColumnRenderers = {
      ...
      rrQty: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          {FGRFPNumericInput(row, index, "rrQty")}
        </td>
      ),
      freeQty: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          {FGRFPNumericInput(row, index, "freeQty")}
        </td>
      ),
      unitCost: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          {FGRFPNumericInput(row, index, "unitCost")}
        </td>
      ),
      grossAmount: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
      ...
      bbDate: () => (
        <td key={columnKey} className="global-tran-td-ui" style={style}>
          <input
            ...
            onKeyDown={(e) => handleFGRFPGridKeyDown(e, index, "bbDate", { readOnly: isFormDisabled })}
          />
        </td>
      ),
      qstatCode: () => (
  ...
  const FGRFPNumericInput = (row, index, field, options = {}) => {
    ...
    return (
      <input
        ...
        onFocus={(e) => {
          ...
          if (typeof clearFGRFPDetailZeroOnFocus === "function") {
            clearFGRFPDetailZeroOnFocus(e, {
              isEditable: true,
        ...
        onKeyDown={(e) => handleFGRFPGridKeyDown(e, index, field, { readOnly, disabled })}
      />
    );
  };

  const renderFGRFPGlCell = (columnKey, row, index) => {
    const columnWidth = getFGRFPGlFallbackWidth(columnKey);
    const style = getFGRFPGlCellStyle(columnKey, columnWidth);
    const rowLocked = isFormDisabled || row.operation === "S";

    const focusNextGlCell = (field) => {
      if (typeof focusNextFGRFPGlRowInput === "function") {
        focusNextFGRFPGlRowInput(index, field, {
          rows: detailRowsGLRef.current || detailRowsGL,
          zeroClearFields: FGRFPGlEnterNextRowZeroClearFields,
          parseValue: parseFormattedNumber,
    ...
    const amountInput = (field) => (
      <input
        ...
        onFocus={(e) => {
          ...
          if (typeof clearFGRFPGlZeroOnFocus === "function") {
            clearFGRFPGlZeroOnFocus(e, {
              isEditable: true,
...
                  <tr>
                    {visibleFGRFPDetailColumns.map((column) =>
                      renderFGRFPDetailHeader(column.label, column.key, column.width, {
                        orderedColumns: visibleFGRFPDetailColumns,
                      })
                    )}
                    {!isFormDisabled && (
...
                <tbody className="relative">
                  {sortedFGRFPDetailRows.map(({ row, originalIndex }) => (
                    <tr key={originalIndex} className="global-tran-tr-ui">
                      {visibleFGRFPDetailColumns.map((column) =>
                        renderFGRFPDetailCell(column.key, row, originalIndex)
                      )}
                      {!isFormDisabled && (
                        <td
              ...
              {renderFGRFPDetailHeaderContextMenu?.()}
            </div>
          </div>
                ...
                {showTypeDropdown && (
                  <div className="absolute bottom-[110%] left-0 mb-2 z-[9999] w-[200px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_10px_24px_rgba(15,23,42,0.16)] backdrop-blur-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <div className="p-1.5">
                      <button
                        type="button"
                        className="mt-1 flex w-full iterm-center justify-between rounded-lg px-2.5 py-1.5 text-xs font-medium text-slate-700 transition-all duration-150 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-100 dark:hover:bg-slate-700"
                        onClick={handleOpenrmLookup}
                      >
                        <div className="flex iterm-center gap-2">
                          <span className="flex h-7 w-7 iterm-center justify-center rounded-lg bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-200">
                            <FontAwesomeIcon icon={faTableCellsLarge} />
                          </span>
                          <div className="flex flex-col iterm-start">
                      ...
                      {/* <button
                        type="button"
                        className="flex w-full iterm-center justify-between rounded-lg px-2.5 py-1.5 text-xs font-medium text-blue-700 transition-all duration-150 hover:bg-blue-50 hover:text-blue-900 dark:text-blue-300 dark:hover:bg-slate-700"
                        onClick={() => {
                          setShowTypeDropdown(false);
                      ...
                      > */}
                        {/* //<div className="flex iterm-center gap-2"> */}
                          {/* <span className="flex h-7 w-7 iterm-center justify-center rounded-lg bg-blue-50 text-blue-600 dark:bg-slate-700 dark:text-blue-300">
            ...
            <div className="global-tran-table-main-div-ui">
              <div className="global-tran-table-main-sub-div-ui">
                <table className="min-w-full border-separate border-spacing-0 [&_th]:border-b [&_th]:border-slate-200 [&_td]:border-t-0 [&_td]:border-l-0 [&_td]:border-r [&_td]:border-b [&_td]:border-slate-200 [&_tr>td:first-child]:border-l">
                  <thead className="global-tran-thead-div-ui">
                    <tr>
                      {orderedFGRFPGlColumns.map((column) =>
                        renderFGRFPGlHeader(column.label, column.key, column.width, {
                          orderedColumns: orderedFGRFPGlColumns,
                        }),
                      )}
                      {/* {!isFormDisabled && (
                        <th
                  ...
                  <tbody className="relative">
                    {sortedFGRFPGlRows.map(({ row, originalIndex }) => (
                      <tr
                        key={`${row.acctCode || row.id || "gl"}-${originalIndex}`}
                        className="global-tran-tr-ui"
                      >
                        {orderedFGRFPGlColumns.map((column) =>
                          renderFGRFPGlCell(column.key, row, originalIndex),
                        )}
                        {/* {!isFormDisabled && (
                          <td
                            className="global-tran-td-ui text-center sticky right-0 bg-white dark:bg-black"
                            style={transactionActionsCellStyle}
                          >
                            <div className="flex iterm-center justify-center gap-1">
                              <button
                                type="button"
                ...
                {renderFGRFPGlHeaderContextMenu?.()}
              </div>
            </div>
...
        <AllTranHistory
          showHeader={false}
          endpoint="/getFGRFPHistory"
          cacheKey={`FGRFP:${state.branchCode || ""}:${state.documentNo || ""}`}
          activeTabKey="FGRFP_Summary"
          branchCode={state.branchCode}
          startDate={null}
          endDate={null}
          status={(() => {
...
      )}
      ...
      {false && showLotPickingModal && selectedLotPickingRow && (
        <div className="fixed inset-0 z-[40] flex iterm-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-6xl rounded-lg border border-gray-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
            <div className="flex iterm-start justify-between gap-3 border-b border-gray-200 px-4 py-3 dark:border-slate-700">
              <div>
  ...
  {/* RR QTY Block */}
  <div className="flex flex-col iterm-center justify-center rounded-md bg-gray-100 px-4 py-1.5 dark:bg-slate-900/50">
  ...
  <div className="flex flex-col iterm-center justify-center rounded-md bg-gray-100 px-4 py-1.5 dark:bg-slate-900/50">
    ...
    >
      {formatNumber(lotBreakdownRemainingQty, decQty)}
            ...
            <div className="flex iterm-center justify-end border-t border-gray-200 px-4 py-3 dark:border-slate-700">
              <div className="flex iterm-center gap-2">
                <button
                  type="button"
...
      )}
      ...
      {rmLookupModalOpen && (
        <ItemMastLookupModal
          ...
          onGetSelectedIterm={handleClosermLookup}
          onCancel={() =>
            updateState({ rmLookupModalOpen: false, selectedRowIndex: null })
          }
          enableMultiSelect
