...
const PostRMST = ({ isOpen, onClose, userCode }) => {
  ...
  const handlePost = async (selectedData, userPw) => {
    await useHandlePostTran(selectedData, userPw, "RMST", userCode, setLoading, onClose);
  };
...
const handleViewDocument = (row) => {
  ...
  const RMST_VIEW_URL = "/page/RMST";
  const url =
    `${window.location.origin}${RMST_VIEW_URL}` +
    `?rmstNo=${encodeURIComponent(docNo)}&branchCode=${encodeURIComponent(branchCode)}`;
