...
const PostFGPR = ({ isOpen, onClose, userCode }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("Error fetching FGPR:", error);
      } finally {
  ...
  // =========================================
    ...
    await useHandlePostTran(selectedData, userPw, "FGPR", userCode, setLoading, onClose);
  };
  ...
  const handleViewDocument = (row) => {
    ...
    if (!docNo || !branchCode) {
      useSwalValidationAlert({
        icon: "warning",
        title: "Missing keys",
        message: "Cannot determine FGPR No or Branch Code"
      });
      return;
    }

    const TRAN_VIEW_URL = "/page/FGPR";
    const url =
      `${window.location.origin}${TRAN_VIEW_URL}` +
      `?fgprNo=${encodeURIComponent(docNo)}` +
    ...
    <>
      {modalReady && (
        <GlobalGLPostingModalv1
          ...
          title="Finalize FG Production Receipt"
