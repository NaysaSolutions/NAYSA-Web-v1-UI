...
const PostRMAJ = ({ isOpen, onClose, userCode }) => {
      ...
      try {
        ...
        if (postingRows.length === 0 && !alertFired.current) {
          useSwalValidationAlert({
            ...
            message: "There are no RM Adjustment records to post.",
          });
        ...
        console.error("Error fetching RMAJ posting data:", error);

        useSwalValidationAlert({
          icon: "error",
          title: "Posting Load Error",
          message: "Unable to load RM Adjustment records for posting.",
        });
  ...
  const pickDocAndBranch = (row) => {
    ...
    const docNo = row.rmajNo || row.docNo || row.RMAJNo || row.RMAJ_NO || null;
  ...
  const handleViewDocument = (row) => {
    ...
    if (!docNo || !branchCode) {
      useSwalValidationAlert({
        icon: "warning",
        title: "Missing Keys",
        message: "Cannot determine RMAJ No. or Branch Code.",
      });
      return;
    }

    const RMAJ_VIEW_URL = "/page/RMAJ";
    const url =
      `${window.location.origin}${RMAJ_VIEW_URL}` +
      `?rmajNo=${encodeURIComponent(docNo)}` +
