...
const FARC = () => {
  ...
  const documentTitle = `${hsDoc?.docName || "FA Reclassification"} Transaction`;
  ...
  useEffect(() => {
    ...
    const filteredTypes = getAllDropDown?.("FARCTRAN_TYPE", docType) || [];
    updateState((prev) => {
      const nextTranType = prev.reclassType || filteredTypes[0]?.DROPDOWN_CODE || "FARC01";
      if (
        prev.reclassType === nextTranType &&
        areDropdownListsEqual(prev.reclassTypeList || [], filteredTypes)
      ) {
        return null;
      }

      return {
        reclassTypeList: filteredTypes,
        reclassType: nextTranType,
      };
    });
  ...
  const loadCompanyData = async () => {
    ...
    try {
      const hdtblcol_result = await fieldLenghtCheck("farc_hd,farc_dt1,farc_dt2");
  ...
  const fetchTranData = async (documentNo, branchCode, direction = "") => {
    const resetState = () => {
      updateState({ farcNo: "", documentID: "", isDocNoDisabled: false, isFetchDisabled: false });
      setDetailRows([]);
      setGlRows([]);
    };
    ...
    try {
      const data = await useFetchTranData(documentNo, branchCode, docType, "farcNo", direction);
      ...
      updateState({
        documentStatus: data.farcHStatus || "",
        status: data.status || "OPEN",
        noReprints: data.noReprints || "0",
        documentID: data.farcId || "",
        farcNo: data.farcNo || "",
        branchCode: data.branchCode || "",
        branchName: data.branchName || "",
        farcDate: useformatToDatev2(data.farcDate || ""),
        reclassType: data.reclassType || "FARC01",
  ...
  const buildTransactionPayload = (nextGlRows = glRows) => ({
    ...
    farcNo: state.farcNo || "",
    farcId: state.documentID || "",
    documentID: state.documentID || "",
    farcDate: state.farcDate || "",
    reclassType: state.reclassType || "FARC01",
      ...
      try {
        ...
        if (action === "Upsert") {
          const requiredFields = {
            ...
            "FARC Date": state.farcDate,
            "Reference No.": state.referenceNo,
          };

          const isValid = await validateRequiredFields(requiredFields, "Save FARC");
          if (!isValid) return;
          ...
          if (response) {
            const responseDocNo = response.data[0].farcNo;
            const responseDocId = response.data[0].farcId;
  ...
  const handleConfirmCancel = async (cancelReason) => {
    const success = await useHandleCancel(docType, state.documentID, state.farcNo, state.branchCode, state.userCode || currentUserRow?.userCode || "", cancelReason);
    updateState({ showCancelModal: false });
    if (success) await fetchTranData(state.farcNo, state.branchCode);
  };
  ...
  const handleTranDocNoRetrieval = async (payload) => {
    await fetchTranData(payload?.docNo || state.farcNo, state.branchCode, payload?.key || "");
  };
                  ...
                  <div className="relative w-full">
                    <div className="flex items-stretch global-ref-textbox-ui global-ref-textbox-enabled">
                      <DateFormatInput
                        ...
                        value={state.farcDate}
                        disabled={isFormDisabled}
                        updateState={(updates) => {
                          if (isFormDisabled) return;
                          updateState({ farcDate: updates.farcDate });
                        }}
                      />
                    </div>
                    <label htmlFor="farcDate" className="global-ref-floating-label global-ref-label-enabled">FARC Date</label>
                  </div>
                  <FieldRenderer
                    id="reclassType"
                    label="Reclass Type"
                    required
                    type="select"
                    value={state.reclassType || "FARC01"}
                    disabled={isFormDisabled}
                    onChange={(val) => updateState({ reclassType: val })}
                    options={(state.reclassTypeList || []).map((type) => ({
