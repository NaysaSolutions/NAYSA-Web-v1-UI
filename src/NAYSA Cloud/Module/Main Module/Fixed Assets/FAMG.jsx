...
const FAMG = () => {
  ...
  const documentTitle = `${hsDoc?.docName || "FA Merge"} Transaction`;
  ...
  useEffect(() => {
    ...
    const filteredTypes = getAllDropDown?.("FAMGTRAN_TYPE", docType) || [];
    updateState((prev) => {
      const nextTranType = prev.mergeType || filteredTypes[0]?.DROPDOWN_CODE || "FAMG01";
      if (
        prev.mergeType === nextTranType &&
        areDropdownListsEqual(prev.mergeTypeList || [], filteredTypes)
      ) {
        return null;
      }

      return {
        mergeTypeList: filteredTypes,
        mergeType: nextTranType,
      };
    });
  ...
  const loadCompanyData = async () => {
    ...
    try {
      const hdtblcol_result = await fieldLenghtCheck("famg_hd,famg_dt1,famg_dt2");
  ...
  const fetchTranData = async (documentNo, branchCode, direction = "") => {
    const resetState = () => {
      updateState({ famgNo: "", documentID: "", isDocNoDisabled: false, isFetchDisabled: false });
      setDetailRows([]);
      setGlRows([]);
    };
    ...
    try {
      const data = await useFetchTranData(documentNo, branchCode, docType, "famgNo", direction);
      ...
      updateState({
        documentStatus: data.famgHStatus || "",
        status: data.status || "OPEN",
        noReprints: data.noReprints || "0",
        documentID: data.famgId || "",
        famgNo: data.famgNo || "",
        branchCode: data.branchCode || "",
        branchName: data.branchName || "",
        famgDate: useformatToDatev2(data.famgDate || ""),
        mergeType: data.mergeType || "FAMG01",
  ...
  const buildTransactionPayload = (nextGlRows = glRows) => ({
    ...
    famgNo: state.famgNo || "",
    famgId: state.documentID || "",
    documentID: state.documentID || "",
    famgDate: state.famgDate || "",
    mergeType: state.mergeType || "FAMG01",
      ...
      try {
        ...
        if (action === "Upsert") {
          const requiredFields = {
            ...
            "FAMG Date": state.famgDate,
            "Reference No.": state.referenceNo,
          };

          const isValid = await validateRequiredFields(requiredFields, "Save FAMG");
          if (!isValid) return;
          ...
          if (response) {
            const responseDocNo = response.data[0].famgNo;
            const responseDocId = response.data[0].famgId;
  ...
  const handleConfirmCancel = async (cancelReason) => {
    const success = await useHandleCancel(docType, state.documentID, state.famgNo, state.branchCode, state.userCode || currentUserRow?.userCode || "", cancelReason);
    updateState({ showCancelModal: false });
    if (success) await fetchTranData(state.famgNo, state.branchCode);
  };
  ...
  const handleTranDocNoRetrieval = async (payload) => {
    await fetchTranData(payload?.docNo || state.famgNo, state.branchCode, payload?.key || "");
  };
                  ...
                  <div className="relative w-full">
                    <div className="flex items-stretch global-ref-textbox-ui global-ref-textbox-enabled">
                      <DateFormatInput
                        ...
                        value={state.famgDate}
                        disabled={isFormDisabled}
                        updateState={(updates) => {
                          if (isFormDisabled) return;
                          updateState({ famgDate: updates.famgDate });
                        }}
                      />
                    </div>
                    <label htmlFor="famgDate" className="global-ref-floating-label global-ref-label-enabled">FAMG Date</label>
                  </div>
                  <FieldRenderer
                    id="mergeType"
                    label="Merge Type"
                    required
                    type="select"
                    value={state.mergeType || "FAMG01"}
                    disabled={isFormDisabled}
                    onChange={(val) => updateState({ mergeType: val })}
                    options={(state.mergeTypeList || []).map((type) => ({
