...
const FAAD = () => {
  ...
  const documentTitle = `${hsDoc?.docName || "FA Adjustment"} Transaction`;
  ...
  useEffect(() => {
    ...
    const filteredTypes = getAllDropDown?.("FAADTRAN_TYPE", docType) || [];
    updateState((prev) => {
      const nextTranType = prev.adjustType || filteredTypes[0]?.DROPDOWN_CODE || "COST";
      if (
        prev.adjustType === nextTranType &&
        areDropdownListsEqual(prev.adjustTypeList || [], filteredTypes)
      ) {
        return null;
      }

      return {
        adjustTypeList: filteredTypes,
        adjustType: nextTranType,
      };
    });
  ...
  const loadCompanyData = async () => {
    ...
    try {
      const hdtblcol_result = await fieldLenghtCheck("faad_hd,faad_dt1,faad_dt2");
  ...
  const fetchTranData = async (documentNo, branchCode, direction = "") => {
    const resetState = () => {
      updateState({ faadNo: "", documentID: "", isDocNoDisabled: false, isFetchDisabled: false });
      setDetailRows([]);
      setGlRows([]);
    };
    ...
    try {
      const data = await useFetchTranData(documentNo, branchCode, docType, "faadNo", direction);
      ...
      updateState({
        documentStatus: data.faadHStatus || "",
        status: data.status || "OPEN",
        noReprints: data.noReprints || "0",
        documentID: data.faadId || "",
        faadNo: data.faadNo || "",
        branchCode: data.branchCode || "",
        branchName: data.branchName || "",
        faadDate: useformatToDatev2(data.faadDate || ""),
        adjustType: data.adjustType || "COST",
  ...
  const buildTransactionPayload = (nextGlRows = glRows) => ({
    ...
    faadNo: state.faadNo || "",
    faadId: state.documentID || "",
    documentID: state.documentID || "",
    faadDate: state.faadDate || "",
    adjustType: state.adjustType || "COST",
      ...
      try {
        ...
        if (action === "Upsert") {
          const requiredFields = {
            ...
            "FAAD Date": state.faadDate,
            "Reference No.": state.referenceNo,
          };

          const isValid = await validateRequiredFields(requiredFields, "Save FAAD");
          if (!isValid) return;
          ...
          if (response) {
            const responseDocNo = response.data[0].faadNo;
            const responseDocId = response.data[0].faadId;
  ...
  const handleConfirmCancel = async (cancelReason) => {
    const success = await useHandleCancel(docType, state.documentID, state.faadNo, state.branchCode, state.userCode || currentUserRow?.userCode || "", cancelReason);
    updateState({ showCancelModal: false });
    if (success) await fetchTranData(state.faadNo, state.branchCode);
  };
  ...
  const handleTranDocNoRetrieval = async (payload) => {
    await fetchTranData(payload?.docNo || state.faadNo, state.branchCode, payload?.key || "");
  };
                  ...
                  <div className="relative w-full">
                    <div className="flex items-stretch global-ref-textbox-ui global-ref-textbox-enabled">
                      <DateFormatInput
                        ...
                        value={state.faadDate}
                        disabled={isFormDisabled}
                        updateState={(updates) => {
                          if (isFormDisabled) return;
                          updateState({ faadDate: updates.faadDate });
                        }}
                      />
                    </div>
                    <label htmlFor="faadDate" className="global-ref-floating-label global-ref-label-enabled">FAAD Date</label>
                  </div>
                  <FieldRenderer
                    id="adjustType"
                    ...
                    required
                    type="select"
                    value={state.adjustType || "COST"}
                    disabled={isFormDisabled}
                    onChange={(val) => updateState({ adjustType: val })}
                    options={(state.adjustTypeList || []).map((type) => ({
