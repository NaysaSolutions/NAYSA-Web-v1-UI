...
const PostFADS = ({ isOpen, onClose, userCode = "" }) => {
  ...
  useEffect(() => {
    ...
    const fetchPostingData = async () => {
      ...
      try {
        ...
        console.error("Error fetching FADS posting data:", error);

        useSwalValidationAlert({
          icon: "error",
          title: "Posting Load Error",
          message: "Unable to load FA Disposal records for posting.",
        });
  ...
  const pickDocAndBranch = (row) => {
    ...
    const docNo = row.fadsNo || row.docNo || row.FADSNo || row.FADS_NO || null;
  ...
  const handleViewDocument = (row) => {
    ...
    if (!docNo || !branchCode) {
      useSwalValidationAlert({
        icon: "warning",
        title: "Missing Keys",
        message: "Cannot determine FADS No. or Branch Code.",
      });
      return;
    }

    const VIEW_URL = "/page/FADS";
    const url =
      `${window.location.origin}${VIEW_URL}` +
      `?fadsNo=${encodeURIComponent(docNo)}` +
