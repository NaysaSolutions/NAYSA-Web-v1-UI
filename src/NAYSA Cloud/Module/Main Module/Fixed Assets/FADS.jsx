...
const FADS = () => {
  ...
  useEffect(() => {
    ...
    updateState((prev) => {
      const nextTranType = prev.disposalType || filteredTypes[0]?.DROPDOWN_CODE || "SALE";
      if (
        prev.disposalType === nextTranType &&
  ...
  const buildTransactionPayload = (nextGlRows = glRows) => ({
    ...
    fadsNo: state.fadsNo || "",
    fadsId: state.documentID || "",
    documentID: state.documentID || "",
    fadsDate: state.fadsDate || "",
    disposalType: state.disposalType || "SALE",
  ...
  const handleConfirmCancel = async (cancelReason) => {
    const success = await useHandleCancel(docType, state.documentID, state.fadsNo, state.branchCode, state.userCode || currentUserRow?.userCode || "", cancelReason);
    updateState({ showCancelModal: false });
    if (success) await fetchTranData(state.fadsNo, state.branchCode);
  };
  ...
  const handleTranDocNoRetrieval = async (payload) => {
    await fetchTranData(payload?.docNo || state.fadsNo, state.branchCode, payload?.key || "");
  };
              ...
              <div className="global-tran-textbox-grid-ui">
                <div className="global-tran-textbox-group-div-ui">
                  ...
                  <FieldRenderer
                    ...
                    required
                    type="select"
                    value={state.disposalType || "SALE"}
                    disabled={isFormDisabled}
                    onChange={(val) => updateState({ disposalType: val })}
