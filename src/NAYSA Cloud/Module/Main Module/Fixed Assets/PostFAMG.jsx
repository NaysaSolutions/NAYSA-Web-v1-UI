...
const PostFAMG = ({ isOpen, onClose, userCode = "" }) => {
      ...
      try {
        ...
        if (postingRows.length === 0 && !alertFired.current) {
          useSwalValidationAlert({
            ...
            message: "There are no FA Merge records to post.",
          });
        ...
        console.error("Error fetching FAMG posting data:", error);

        useSwalValidationAlert({
          icon: "error",
          title: "Posting Load Error",
          message: "Unable to load FA Merge records for posting.",
        });
  ...
  const pickDocAndBranch = (row) => {
    ...
    const docNo = row.famgNo || row.docNo || row.FAMGNo || row.FAMG_NO || null;
  ...
  const handleViewDocument = (row) => {
    ...
    if (!docNo || !branchCode) {
      useSwalValidationAlert({
        icon: "warning",
        title: "Missing Keys",
        message: "Cannot determine FAMG No. or Branch Code.",
      });
      return;
    }

    const VIEW_URL = "/page/FAMG";
    const url =
      `${window.location.origin}${VIEW_URL}` +
      `?famgNo=${encodeURIComponent(docNo)}` +
