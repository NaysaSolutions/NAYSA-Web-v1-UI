...
import { useHandlePrintFGINVReport, useHandleDownloadExcelFGINVReport } from "@/NAYSA Cloud/Global/report";
import { useTopHSRptRow, useTopUserRow } from "@/NAYSA Cloud/Global/top1RefTable";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faMagnifyingGlass, faXmark, faCircleNotch, faRotateLeft, faBroom, faDownload, faCircleXmark } from "@fortawesome/free-solid-svg-icons";
...
const FGINVReportModal = ({ isOpen, onClose, userCode, closeOnBackdrop = true }) => {
 ...
 // Keyboard handlers (ESC to close, focus trap with Tab)
  ...
  // Field handlers
  const handleCloseBranchModal = (payload) => {
    if (payload) {
      updateState({ branchCode: payload.branchCode, branchName: payload.branchName });
    }
    setBranchModalOpen(false);
  };

  const handleCloseItemModal = (payload) => {
    if (payload) {
      updateState({
  ...
  const handleCloseWarehouseModal = (payload) => {
    if (payload) {
      updateState({
        whCode: payload.whCode || "",
        whName: payload.whName || "",
        locCode: "",
        locName: "",
      });
    }
  ...
  const handleCloseLocationModal = (payload) => {
    if (payload) {
      updateState({
        locCode: payload.locCode || "",
        locName: payload.locName || "",
        whCode: payload.whCode || filters.whCode,
      });
    }
  ...
  const clearItem = () => updateState({ itemCode: "", itemName: "" });
  const clearWarehouse = () => updateState({ whCode: "", whName: "", locCode: "", locName: "" });
  const clearLocation = () => updateState({ locCode: "", locName: "" });

  const handleReset = () => {
    updateState({ itemCode: "", itemName: "", whCode: "", whName: "", locCode: "", locName: "" });
  };
...
const handlePreview = async () => {
  try {
    ...
    const response = meta.export === "Y"
      ? await useHandleDownloadExcelFGINVReport(params)
      : await useHandlePrintFGINVReport(params);
 ...
 return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b bg-gradient-to-r from-white to-blue-100">
          <h2 id="ap-report-title" className="text-sm md:text-base font-semibold text-blue-700">
            FG Inventory Reports
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-2 p-2 md:p-4 overflow-hidden">
          ...
          <div className="md:col-span-2 border rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="p-3 md:p-5 space-y-2 md:space-y-3 overflow-y-auto">
              ...
              {/* Item Code */}
              <div className="grid grid-cols-1 md:grid-cols-[7.5rem_1fr] items-center gap-2 md:gap-4">
                <label className="text-xs md:text-sm font-medium">Item Code</label>
                <div className="relative">
                  <input
                    type="text"
                    readOnly
              ...
              {/* Warehouse */}
              <div className="grid grid-cols-1 md:grid-cols-[7.5rem_1fr] items-center gap-2 md:gap-4">
                <label className="text-xs md:text-sm font-medium">Warehouse</label>
                <div className="relative">
                  <input
                    type="text"
                    readOnly
...
export default FGINVReportModal;
