...
const SearchFAClass = ({
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let result = assetClasses.filter((item) => {
      return (
        (item.code || "")
          .toLowerCase()
    ...
    return result;
  }, [assetClasses, debouncedFilters, sortConfig]);
            ...
            <table className="min-w-full border-separate border-spacing-0 table-fixed">
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedData.length > 0 ? (
                  paginatedData.map((assetClass, index) => (
                    <tr
                      key={assetClass.code || index}
                      onClick={() => handleApply(assetClass)}
