...
const SearchFGInvClass = ({
  isOpen,
  onClose,
  title = "Search FG Inventory Classification Codes",
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let result = classifications.filter((item) => {
      return (
        (item.code || "")
          .toLowerCase()
    ...
    return result;
  }, [classifications, debouncedFilters, sortConfig]);
  ...
  const handleApply = (classification) => {
          ...
          {isLoading ? (
            ...
            <table className="min-w-full border-separate border-spacing-0 table-fixed">
              <thead className="sticky top-0 z-10 bg-slate-200">
                <tr>
                  {[
                    { label: "Classification Code",       key: "code",        width: "w-[150px]" },
                    { label: "Classification Description", key: "description"                     },
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedData.length > 0 ? (
                  paginatedData.map((classification, index) => (
                    <tr
                      key={classification.code || index}
                      onClick={() => handleApply(classification)}
                      className="group hover:bg-blue-50 cursor-pointer transition-colors"
                    >
                      <td className="global-lookup-td-ui font-bold">{classification.code}</td>
                      <td className="global-lookup-td-ui">{classification.description}</td>
                      <td className="global-lookup-td-ui">{classification.categCode}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
...
export default SearchFGInvClass;