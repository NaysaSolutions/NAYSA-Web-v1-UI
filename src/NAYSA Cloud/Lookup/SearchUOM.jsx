...
const SearchUOM = ({
  ...
  const {
    ...
    queryKey: ["lookupUom", customParam],
    queryFn: async () => {
      const { data: result } = await apiClient.get("/lookupUom", {
  ...
  const filteredData = useMemo(() => {
    ...
    return uomList.filter((item) => {
      return (
        (item.uomCode || "").toLowerCase().includes(debouncedFilters.uomCode.toLowerCase()) &&
        (item.uomName || "").toLowerCase().includes(debouncedFilters.uomName.toLowerCase())
      );
    });
  }, [uomList, debouncedFilters]);
              ...
              <thead className="sticky top-0 z-10 bg-slate-200 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left">
                    ...
                    <input
                      type="text"
                      value={filters.uomName}
                      onChange={(e) => setFilters(p => ({ ...p, uomName: e.target.value }))}
                      placeholder="Filter..."
                      className="w-full px-2 py-1 text-xs border rounded bg-white dark:bg-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20"
                    />
                  </th>
