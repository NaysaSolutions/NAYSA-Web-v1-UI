...
const SearchPayTermRef = ({ isOpen, onClose }) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return parsedData.map((item) => ({
        ...item,
        paytermCode: item.paytermCode ?? item.payterm_code ?? "",
        paytermName: item.paytermName ?? item.payterm_name ?? "",
        daysDue: item.daysDue ?? item.days_due ?? "",
      }));
    },
    enabled: isOpen,
    staleTime: 0,
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let result = payterms.filter((item) => {
      const paytermCode = String(item?.paytermCode ?? "").toLowerCase();
      const paytermName = String(item?.paytermName ?? "").toLowerCase();
      const daysDue = String(item?.daysDue ?? "").toLowerCase();

      return (
        paytermCode.includes(debouncedFilters.paytermCode.toLowerCase()) &&
        paytermName.includes(debouncedFilters.paytermName.toLowerCase()) &&
        daysDue.includes(debouncedFilters.daysDue.toLowerCase())
      );
    });
    ...
    return result;
  }, [payterms, debouncedFilters, sortConfig]);
        ...
        <div className="flex-grow overflow-auto custom-scrollbar bg-white">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-64 text-slate-400">
              ...
              />
              <p className="text-sm">Loading payment term data...</p>
            </div>
          ) : (
            <table className="min-w-full border-separate border-spacing-0">
              <thead className="sticky top-0 z-10 bg-slate-200">
                <tr>
                  {[
                    { label: "Payment Term", key: "paytermCode" },
                    { label: "Description", key: "paytermName" },
                    { label: "Due Days", key: "daysDue" },
                  ].map((col) => (
                    <th
                      key={col.key}
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2 text-xs text-slate-500 w-[140px] text-right">
                        {row.daysDue}
                      </td>
                    </tr>
                  ))
