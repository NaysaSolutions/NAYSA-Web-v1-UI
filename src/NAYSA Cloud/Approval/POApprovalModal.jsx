...
const ENDPOINT = "getPOApproval";
const APPROVE_ENDPOINT = "approvePO";
const EMPTY_PARAMS = {};
...
const getPODisplayNo = (row, index) =>
  String(
    row?.docNo ||
      row?.poNo ||
      row?.PO_NO ||
...
const buildApprovedPOMessage = (approvalRows) => {
  const limit = 5;

  const visibleRows = approvalRows.slice(0, limit);

  const approvedList = visibleRows
    .map((row, index) => `${index + 1}. ${getPODisplayNo(row, index)}`)
    .join("\n");
  ...
  return `The following PO${
    approvalRows.length > 1 ? "s have" : " has"
  } been approved:\n${approvedList}${moreText}`;
};

const buildDisapprovedPOMessage = (approvalRows) => {
  const limit = 5;
...
const buildCommentedPOMessage = (approvalRows) => {
  const limit = 5;
  ...
  return `Approver's note has been applied to the following PO${
    approvalRows.length > 1 ? "s" : ""
  }:\n${commentedList}${moreText}`;
};

const POApprovalModal = ({
  isOpen,
  approverName,
  department,
  userCode,
  ...
  detailRows,
  detailColumns,
  transactionLabel = "Purchase Order Approval",
  ...
  const approvalLevel = currentUserRow?.poAppLevel ||  "";
  ...
  const loadColumns = useCallback(async () => {
    ...
    try {
      ...
      console.error("Load PO approval columns failed:", error);
      loadedColumnsRef.current = true;
    }
  }, [detailColumns]);
  ...
  const showNoApprovalAvailableAlert = useCallback(() => {
    ...
    useSwalInfoAlert(
      "PO Approval",
      "There is no PO for approval available.",
    );
    onClose?.();
  }, [onClose]);
  ...
  const reloadApprovalRows = useCallback(async ({ showLoading = true } = {}) => {
    ...
    try {
      ...
      console.error("Reload PO approval failed:", error);
      setRows([]);
      useSwalErrorAlert(
        "PO Approval",
        error?.response?.data?.message ||
          error?.message ||
          "Unable to reload PO approval detail.",
      );
      return [];
    } finally {
    ...
    async (approvalRows) => {
      ...
      try {
        ...
        await useSwalSuccessAlert(
          "PO Approved",
          buildApprovedPOMessage(targetRows),
        );
        await reloadApprovalRows({ showLoading: false });
      } catch (error) {
        console.error("Approve PO failed:", error);
        useSwalErrorAlert(
          "PO Approval",
          error?.response?.data?.message ||
            error?.message ||
            "Unable to approve selected PO transaction.",
        );
      } finally {
  ...
  const handleDisapproveRows = useCallback(
    async (approvalRows, reason) => {
      ...
      try {
        ...
        await useSwalSuccessAlert(
          "PO Disapproved",
          buildDisapprovedPOMessage(targetRows),
        );
        await reloadApprovalRows({ showLoading: false });
        return true;
      } catch (error) {
        console.error("Disapprove PO failed:", error);
        useSwalErrorAlert(
          "PO Approval",
          error?.response?.data?.message ||
            error?.message ||
            "Unable to disapprove selected PO transaction.",
        );
        return false;
      } finally {
        setIsApproving(false);
    ...
    async (approvalRows, note) => {
      ...
      try {
        ...
        await useSwalSuccessAlert(
          ...
          buildCommentedPOMessage(targetRows),
        );
        await reloadApprovalRows({ showLoading: false });
        return true;
      } catch (error) {
        console.error("Apply PO approver note failed:", error);
        useSwalErrorAlert(
          "PO Approval",
          error?.response?.data?.message ||
            error?.message ||
            "Unable to apply approver's note to selected PO transaction.",
        );
        return false;
      } finally {
        setIsApproving(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error("Fetch PO approval failed:", error);
        if (alive) {
          setRows([]);
          setIsInitialLoadComplete(true);
          useSwalErrorAlert(
            ...
            error?.response?.data?.message ||
              error?.message ||
              "Unable to load PO approval detail.",
          );
        }
      } finally {
